...
case $host in
  ...
  *linux*|*freebsd*)
    if test "$PIE_ENABLE" = "yes"; then
      ...
      PIE_LINK='$(LINK)'
      PIE_NAME='$(LIBNAME)'
      PIE_LDFLAGS="-shared -Xlinker -soname=${PROJECT_NAME}.${PIE_EXEEXT}.${VERSION_MAJOR}"
  ...
  *darwin*)
    if test "$PIE_ENABLE" = "yes"; then
      ...
      PIE_LINK='$(LINK)'
      PIE_NAME='$(LIBNAME)'
      PIE_LDFLAGS="-dynamiclib -undefined error -install_name \$(libdir)/$PROJECT_NAME.$VERSION_MAJOR.$VERSION_MINOR.$VERSION_PATCH.$PIE_EXEEXT -compatibility_version $VERSION_MAJOR.$VERSION_MINOR -current_version $VERSION_MAJOR.$VERSION_MINOR.$VERSION_PATCH"
