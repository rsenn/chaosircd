...
struct user *user_new(const char *name, const char *uid) {
	...
	dlink_add_tail(&user_lists[uptr->uhash % USER_HASH_SIZE], &uptr->hnode,
			uptr);
...
void user_dump(struct user *uptr) {
	if(uptr == NULL) {
		...
		dlink_foreach_data(&user_list, node, uptr)
			dump(user_log, " #%u: [%u] %-20s %-8s (%s)", uptr->id,
					uptr->refcount, uptr->name[0] ? uptr->name : "<unknown>",
					uptr->uid, uptr->client ? uptr->client->name : "?");
