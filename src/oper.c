...
/* -------------------------------------------------------------------------- *
 * -------------------------------------------------------------------------- */
static void oper_drain_cb(uint64_t src, int lvl, const char *level,
  ...
  dlink_foreach(&oper_list, optr) {
    if ((optr->sources & src) && (optr->level >= lvl)) {
      dlink_foreach_data(&optr->online, node, cptr) {
        client_send(cptr, ":%s NOTICE %s :<%s> - %s", client_me->name,
                    cptr->name, source, msg);
      }
    }
  }
}
...
void oper_shutdown(void) {
  ...
  dlink_foreach_safe(&oper_list, node, next) oper_delete((struct oper *)node);
...
/* -------------------------------------------------------------------------- *
 * -------------------------------------------------------------------------- */
struct oper *oper_add(const char *name, const char *passwd, struct class *clptr,
                      int level, uint64_t sources, uint32_t flags) {
  struct oper *optr;
...
/* -------------------------------------------------------------------------- *
 * -------------------------------------------------------------------------- */
int oper_update(struct oper *optr, const char *passwd, struct class *clptr,
                int level, uint64_t sources, uint32_t flags) {
  strlcpy(optr->passwd, passwd, sizeof(optr->passwd));
...
struct oper *oper_pop(struct oper *optr) {
  if (optr) {
    if (!optr->refcount)
      log(oper_log, L_warning, "Poping deprecated oper: %s", optr->name);
...
void oper_up(struct oper *optr, struct client *cptr) {
  ...
  log(oper_log, L_status, "%s (%s@%s) has become an IRC operator.", cptr->name,
