 ...
 * -------------------------------------------------------------------------- */
int usermode_register(struct usermode *umptr) {
  int table_index;

  if (!USERMODE_VALID(umptr->character)) {
    log(usermode_log, L_warning,
        "usermode register failed: "
        "invalid usermode %c",
        umptr->character);
...
void usermode_assemble(uint64_t modes, char *umbuf) {
  ...
  dlink_foreach_data(&usermode_list, n, umptr) if (umptr->flag & modes) *p++ =
      umptr->character;
 ...
 * -------------------------------------------------------------------------- */
int usermode_parse(uint64_t modes, char **args, struct client *cptr,
                   uint32_t flags) {
 ...
 * -------------------------------------------------------------------------- */
void usermode_send_local(struct lclient *lcptr, struct client *cptr,
 ...
 * -------------------------------------------------------------------------- */
void usermode_send_remote(struct lclient *lcptr, struct client *cptr,
                          char *umcbuf, char *umcargs) {
  char noargs = '\0';
  ...
  server_send(lcptr, NULL, CAP_UID, CAP_NONE, ":%s UMODE %s%s",
              client_is_user(cptr) ? cptr->user->uid : cptr->name, umcbuf,
              umcargs);

  server_send(lcptr, NULL, CAP_NONE, CAP_UID, ":%s UMODE %s%s", cptr->name,
              umcbuf, umcargs);
}

void usermode_change_send(struct lclient *lcptr, struct client *cptr,
                          int remote) {
  struct node *node;
  struct usermodechange *umcptr;
...
/* -------------------------------------------------------------------------- *
 * -------------------------------------------------------------------------- */
int usermode_make(struct user *uptr, char **umbuf, struct client *cptr,
                  uint32_t flags) {
  int ret = 0;
 ...
 * Dump usermodes                                                             *
 * -------------------------------------------------------------------------- */
void usermode_dump(struct usermode *umptr) {
  if (umptr == NULL) {
    struct node *n;
    struct usermode *um;
    ...
    if (umptr->list_mode == USERMODE_LIST_NOLIST) {
      ...
      case USERMODE_LIST_ON:
        modestr = "on";
        break;
      case USERMODE_LIST_OFF:
        modestr = "off";
        break;
      }

      switch (umptr->list_type) {
      case USERMODE_LIST_GLOBAL:
        typestr = "global";
        break;
      case USERMODE_LIST_LOCAL:
        typestr = "local";
        break;
      case USERMODE_LIST_REMOTE:
        typestr = "remote";
        break;
      }
