...
/* -------------------------------------------------------------------------- *
 * -------------------------------------------------------------------------- */
static void client_format_cb(char **pptr, size_t *bptr, size_t n, int padding,
                             int left, void *arg) {
...
/* -------------------------------------------------------------------------- *
 * -------------------------------------------------------------------------- */
static void client_nformat_cb(char **pptr, size_t *bptr, size_t n, int padding,
                              int left, void *arg) {
  struct client *acptr = arg;
  char *name;
...
/* -------------------------------------------------------------------------- *
 * -------------------------------------------------------------------------- */
static void client_hformat_cb(char **pptr, size_t *bptr, size_t n, int padding,
                              int left, void *arg) {
  struct client *acptr = arg;
  char *host;
...
/* -------------------------------------------------------------------------- *
 * -------------------------------------------------------------------------- */
static void client_uformat_cb(char **pptr, size_t *bptr, size_t n, int padding,
                              int left, void *arg) {
  struct client *acptr = arg;
  char *user;
...
void client_init(void) {
  ...
  mem_static_create(&client_history_heap, sizeof(struct history),
                    CLIENT_BLOCK_SIZE);
...
void client_shutdown(void) {
  ...
  dlink_foreach_safe(&client_list, cptr, next) client_delete(cptr);
...
struct client *client_new_local(int type, struct lclient *lcptr) {
  ...
  if (type == CLIENT_USER) {
    dlink_add_head(&client_listsn[cptr->hash % CLIENT_HASH_SIZE], &cptr->nnode,
                   cptr);
...
struct client *client_new_remote(int type, struct lclient *lcptr,
                                 ...
                                 const char *name, int hops, const char *host,
                                 const char *hostip, const char *info) {
  struct client *cptr;
...
struct client *client_find_id(uint32_t id) {
  ...
  dlink_foreach(&client_list, cptr) if (cptr->id == id) return cptr;
...
struct history *client_history_add(struct client *cptr, const char *nick) {
  ...
  if ((hptr = client_history_find(nick)) == NULL) {
    ...
    if (client_history[hptr->hash % CLIENT_HASH_SIZE].size ==
        CLIENT_HISTORY_SIZE)
      client_history_delete(
          client_history[hptr->hash % CLIENT_HASH_SIZE].tail->data);

    dlink_add_head(&client_history[hptr->hash % CLIENT_HASH_SIZE], &hptr->node,
                   hptr);
  }
...
void client_history_clean(struct client *cptr) {
  ...
  for (i = 0; i < CLIENT_HASH_SIZE; i++) {
    dlink_foreach(&client_history[i], hptr) if (hptr->client == cptr)
...
void client_lusers(struct client *client) {
  numeric_send(client, RPL_LUSERCLIENT,
               client_lists[CLIENT_GLOBAL][CLIENT_USER].size, 0,
  ...
  numeric_send(client, RPL_STATSCONN, lclient_max - 1, client_max - 1,
               lclient_id - 1);
}

/* -------------------------------------------------------------------------- *
 * Exit a client of any type source this server. * This also generates all
 * necessary protocol messages that this exit may     * cause. *
 * -------------------------------------------------------------------------- */
void client_vexit(struct lclient *lcptr, struct client *cptr,
  ...
  if (client_is_user(cptr)) {
    server_send(lcptr, NULL, CAP_UID, CAP_NONE, ":%s QUIT :%s", cptr->user->uid,
                cstr);
    server_send(lcptr, NULL, CAP_NONE, CAP_UID, ":%s QUIT :%s", cptr->name,
                cstr);

    chanuser_send(lcptr, cptr, ":%s!%s@%s QUIT :%s", cptr->name,
                  cptr->user->name, cptr->host, cstr);
...
int client_nick(struct lclient *lcptr, struct client *cptr, char *nick) {
  ...
  if (cptr->user) {
    chanuser_send(NULL, cptr, ":%N!%U@%H NICK :%s", cptr, cptr, cptr, nick);

    server_send(lcptr, NULL, CAP_UID, CAP_NONE, ":%s NICK %s :%lu",
                cptr->user->uid, nick, (unsigned long)cptr->ts);
    server_send(lcptr, NULL, CAP_NONE, CAP_UID, ":%s NICK %s :%lu", cptr->name,
...
void client_nick_random(char *buf) {
  ...
  do {
    ...
    buf[i++] =
        client_randchars[(client_random() % (sizeof(client_randchars - 12))) +
                         11];
...
void client_introduce(struct lclient *lcptr, struct client *cptr,
  ...
  if (cptr) {
    server_send(lcptr, NULL, CAP_UID, CAP_NONE,
                ":%N NUSER %N %d %lu %s %U %s %s %N %s :%s", cptr, acptr,
                acptr->hops + 1, (unsigned long)acptr->ts,
                acptr->user->mode[0] ? acptr->user->mode : "-", acptr,
                acptr->hostreal, acptr->hostip, acptr->origin, acptr->user->uid,
                acptr->info);

    server_send(lcptr, NULL, CAP_NONE, CAP_UID,
                ":%N NICK %N %d %lu %s %U %s %N :%s", cptr, acptr,
                acptr->hops + 1, (unsigned long)acptr->ts,
                acptr->user->mode[0] ? acptr->user->mode : "-", acptr,
                acptr->hostreal, acptr->origin, acptr->info);
  } else {
    server_send(
        lcptr, NULL, CAP_UID, CAP_NONE, "NUSER %N %d %lu %s %U %s %s %N %s :%s",
        acptr, acptr->hops + 1, (unsigned long)acptr->ts,
        acptr->user->mode[0] ? acptr->user->mode : "-", acptr, acptr->hostreal,
        acptr->hostip, acptr->origin, acptr->user->uid, acptr->info);

    log(server_log, L_debug, "%s: mode: %s", __FUNCTION__, acptr->user->mode);

    server_send(lcptr, NULL, CAP_NONE, CAP_UID,

                "NICK %N %d %lu %s %U %s %N :%s", acptr, acptr->hops + 1,
...
void client_burst(struct lclient *lcptr, struct client *acptr) {
  ...
  log(client_log, L_verbose, "Bursting client %s to server %s", acptr->name,
      lcptr->name);

  if (lcptr->caps & CAP_UID) {
    lclient_send(lcptr, "NUSER %N %d %lu %s %U %s %s %s %s :%s", acptr,
                 acptr->hops + 1, (unsigned long)acptr->ts,
                 acptr->user->mode[0] ? acptr->user->mode : "-", acptr,
                 acptr->hostreal, acptr->hostip, acptr->origin->name,
                 acptr->user->uid, acptr->info);
  } else {
    lclient_send(lcptr, "NICK %N %d %lu %s %s %s %s :%s", acptr,
                 acptr->hops + 1, (unsigned long)acptr->ts,
                 acptr->user->mode[0] ? acptr->user->mode : "-",
                 acptr->user->name, acptr->hostreal, acptr->origin->name,
                 acptr->info);
  }
...
int client_relay(struct lclient *lcptr, struct client *cptr,
  ...
  client_send(acptr, format, cptr, argv[2], argv[3], argv[4], argv[5], argv[6],
              argv[7], argv[8]);
  return 1;
}
...
int client_relay_always(struct lclient *lcptr, struct client *cptr,
                        ...
                        const char *format, int *argc, char **argv) {
  struct client *acptr;
...
void client_message(struct lclient *lcptr, struct client *cptr,
  ...
  if (client_is_local(acptr) && client_is_user(acptr)) {
    if (client_is_user(cptr))
      client_send(acptr, ":%N!%U@%H %s %N :%s", cptr, cptr, cptr, cmd, acptr,
                  text);
    else
      client_send(acptr, ":%N %s %N :%s", cptr, cmd, acptr, text);
...
struct client *client_push(struct client **cptrptr) {
  if (*cptrptr) {
    if ((*cptrptr)->refcount == 0) {
      debug(client_log, "Trying to push deprecated client %N", *cptrptr);
    } else {
      if (--(*cptrptr)->refcount == 0)
...
void client_dump(struct client *cptr) {
  if (cptr == NULL) {
    ...
    dlink_foreach_data(&client_lists[CLIENT_GLOBAL][CLIENT_SERVER], node, cptr)
        dump(client_log, " #%u: [%u] %-20s (%s)", cptr->id, cptr->refcount,
    ...
    dump(client_log, "     origin: %C [%i]", cptr->origin,
         cptr->origin ? cptr->origin->refcount : 0);
    dump(client_log, "       user: %U [%i]", cptr,
         cptr->user ? cptr->user->refcount : 0);
    dump(client_log, "     server: %S [%i]", cptr->server,
