...
struct class *class_add(const char *name, uint64_t ping_freq,
		uint32_t max_clients, uint32_t clients_per_ip, uint32_t recvq,
		uint32_t sendq, uint32_t flood_trigger, uint64_t flood_interval,
	...
	if(clptr == NULL) {
		...
		dlink_foreach_data(&class_list, node, clptr)
			dump(class_log, " #%03u: [%u] %-20s (sendq: %u/recvq: %u)",
					clptr->id, clptr->refcount, clptr->name, clptr->sendq,
					clptr->recvq);
