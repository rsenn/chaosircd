...
static int conf_getopt(int argc, char **argv, const char *optstring,
	...
	if(conf_optind > argc || !argv[conf_optind] || argv[conf_optind][0] != '-'
				...
				if(o->has_arg > 0) {
					...
					/* The conf_optarg is in next argv[] */
						...
						if(!conf_optarg && o->has_arg == 1) {
							...
							log(conf_log, L_fatal, "argument required: `%s'.",
									arg);
							conf_optind++;
	...
	if((tmp = str_chr(optstring, conf_optopt))) {
		...
		if(tmp[1] == ':') {
			...
			if(!conf_optarg) {
				...
				log(conf_log, L_fatal, "missing argument for -%c.",
						conf_optopt);
...
static void usage(char **argv) {
	puts(PACKAGE_NAME"-"PACKAGE_VERSION " - pi-networks irc server\n");
 ...
 * -------------------------------------------------------------------------- */
static struct option longoptions[] = { { "help", 0, NULL, 'h', }, { "nofork", 1,
NULL, 'd', }, { "config", 1, NULL, 'f', }, { NULL, 0, NULL, '\0' } };
 ...
 * -------------------------------------------------------------------------- */
void conf_done(void) {
	memcpy(&conf_current.logs, &conf_new.logs,
			sizeof(struct config) - sizeof(struct conf_global));
...
void conf_read_callback(int fd, void *ptr) {
	...
	if(io_list[fd].status.eof) {
		log(conf_log, L_status, "Read %u lines from %s.",
				io_list[fd].recvq.lines, conf_current.global.configfile);
...
void conf_yy_error(char *msg) {
	log(conf_log, L_warning, "%s:%u: %s: %s", io_list[conf_fd].note, lineno + 1,
			msg, linebuf);
}
