...
struct chanmodechange *chanmode_change_add(struct list *list, int what,
		char mode, char *arg, struct chanuser *acuptr) {
...
struct chanmodechange *chanmode_change_insert(struct list *list,
		struct chanmodechange *before, int what, char mode, char *arg) {
	struct chanmodechange *cmcptr = NULL;
...
struct chanmodechange *chanmode_prepare(struct lclient *lcptr,
		struct client *cptr, struct channel *chptr, int what,
		struct chanmode *mode, char *arg, uint32_t *aiptr) {
...
void chanmode_parse(struct lclient *lcptr, struct client *cptr,
		...
		char *modes, char *args, uint32_t pc) {
	struct chanmodechange *change;
	struct chanmode *mode;
	uint32_t mi;
	uint32_t ci = 0;
	...
	for(mi = 0; modes[mi]; mi++) {
		switch(modes[mi]) {
			...
			default: {
				...
				change = chanmode_prepare(lcptr, cptr, chptr, what, mode,
						argp[ai], &ai);
				if(change)
	...
	dlink_foreach(lptr, cmcptr)
	{
		...
		if(cuptr && cmcptr->what != CHANMODE_QUERY) {
			if(!(cuptr->flags & cmcptr->mode->need)
				...
				if(!denied) {
					numeric_send(cptr, ERR_CHANOPRIVSNEEDED,
							cuptr->channel->name);
					denied++;
				}
			...
			case -2: {
				if(!full) {
					if(client_is_user(cptr) && client_is_local(cptr))
						numeric_send(cptr, ERR_BANLISTFULL, chptr->name,
								cmcptr->arg);
...
void chanmode_send_local(struct client *cptr, struct channel *chptr,
	...
	if(client_is_user(cptr)) {
		...
		len = str_snprintf(cmd, sizeof(cmd), ":%s MODE %s", cptr->name,
				chptr->name);
	}
	...
	for(node = nptr; node;) {
		...
		if(flags[0]) {
			if(args[0]) {
				channel_send(NULL, chptr, CHFLG(NONE), CHFLG(NONE), "%s %s %s",
						cmd, flags, args);
			} else {
				channel_send(NULL, chptr, CHFLG(NONE), CHFLG(NONE), "%s %s",
						cmd, flags);
			}
		}
	}
...
void chanmode_send_remote(struct lclient *lcptr, struct client *cptr,
	...
	for(node = nptr; node;) {
		...
		if(flags[0]) {
			if(args[0])
				server_send(lcptr, NULL, CAP_UID, CAP_NONE, "%s %s %s", cmd,
						flags, args);
			else
				server_send(lcptr, NULL, CAP_UID, CAP_NONE, "%s %s", cmd,
						flags);
		}
	}
	...
	for(node = nptr; node;) {
		...
		if(flags[0]) {
			if(args[0]) {
				server_send(lcptr, NULL, CAP_NONE, CAP_UID, "%s %s %s", cmd,
						flags, args);
			} else {
				server_send(lcptr, NULL, CAP_NONE, CAP_UID, "%s %s", cmd,
						flags);
			}
		}
	}
...
uint64_t chanmode_prefix_parse(const char *pfx) {
	...
	for(i = 0; pfx[i]; i++) {
		...
		for(j = 0; j < sizeof(chanmode_table) / sizeof(chanmode_table[0]);
				j++) {
...
void chanmode_show(struct client *cptr, struct channel *chptr) {
	...
	if(chptr->modes) {
		di += chanmode_flags_build(&mbuf[di], CHANMODE_TYPE_SINGLE,
				chptr->modes);
...
void chanmode_list(struct client *cptr, struct channel *chptr, char c) {
	...
	dlink_foreach(&chptr->modelists[index], cmiptr)
		numeric_send(cptr, numeric, chptr->name, cmiptr->mask, cmiptr->info,
				cmiptr->ts);
...
struct node *chanmode_assemble_list(char *buf, struct node *nptr, size_t len) {
	...
	do {
		...
		if(lasthash && cmcptr->info && lasthash == cmcptr->ihash) {
			if(flen + ilen + tlen + mlen + 2 + 1 + 2 + 12 + str_len(cmcptr->arg)
					+ 2 + 1 > len || flen == CHANMODE_PER_LINE)
				break;
		...
		if(cmcptr->arg[0])
			mlen += strlcpy(&maskbuf[mlen], cmcptr->arg,
					sizeof(maskbuf) - mlen - 1);
		else
			maskbuf[mlen++] = '-';
	...
	if(flen) {
		...
		str_snprintf(buf, len, "%s %s %s %s", flagbuf, infobuf, timebuf,
				maskbuf);
	}
...
size_t chanmode_burst(struct lclient *lcptr, struct channel *chptr) {
	...
	for(i = 0; i < 0x40; i++) {
		...
		if(mode->type == CHANMODE_TYPE_SINGLE) {
			if(chptr->modes & mode->flag)
				chanmode_change_add(&modelist, CHANMODE_ADD, i + 0x40, NULL,
						NULL);
		}
	}
