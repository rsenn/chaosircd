...
struct chanmode *chanmode_register(struct chanmode *cmptr) {
  ...
  if (mode->type != 0) {
    log(chanmode_log, L_warning, "Channel mode char '%c' already registered.",
        cmptr->letter);
    return NULL;
  }
...
int chanmode_unregister(struct chanmode *cmptr) {
  ...
  if (mode->type == 0) {
    log(chanmode_log, L_warning, "Channel mode char '%c' not registered.",
        cmptr->letter);
    return -1;
  }
...
/* -------------------------------------------------------------------------- *
 * -------------------------------------------------------------------------- */
struct chanmodechange *chanmode_change_add(struct list *list, int what,
                                           char mode, char *arg,
...
struct chanmodechange *chanmode_prepare(struct lclient *lcptr,
                                        struct client *cptr,
                                        struct channel *chptr, int what,
                                        struct chanmode *mode, char *arg,
                                        uint32_t *aiptr) {
  ...
  case CHANMODE_TYPE_PRIVILEGE: {
    if (what != CHANMODE_QUERY) {
      ...
      change.target = chanuser_find(chptr, change.acptr);
...
void chanmode_parse(struct lclient *lcptr, struct client *cptr,
                    ...
                    struct list *lptr, char *modes, char *args, uint32_t pc) {
  struct chanmodechange *change;
  struct chanmode *mode;
  uint32_t mi;
  uint32_t ci = 0;
  ...
  for (mi = 0; modes[mi]; mi++) {
    ...
    default: {
      ...
      change = chanmode_prepare(lcptr, cptr, chptr, what, mode, argp[ai], &ai);
      if (change)
  ...
  for (i = 0; i < 0x40; i++) {
    if (CHANMODE_TYPE_KEY & chanmode_table[i].type) {
      if (chptr->modes & chanmode_table[i].flag)
        hooks_call(chanmode_args_build, HOOK_DEFAULT, dst, chptr, &di,
                   &chanmode_table[i].flag);
    }
  }
...
uint32_t chanmode_apply(struct lclient *lcptr, struct client *cptr,
  ...
  dlink_foreach(lptr, cmcptr) {
    ...
    if (cuptr && cmcptr->what != CHANMODE_QUERY) {
      if (!(cuptr->flags & cmcptr->mode->need) && client_is_user(cptr) &&
    ...
    if (cuptr == NULL && cmcptr->what != CHANMODE_QUERY &&
        client_is_user(cptr)) {
      log(chanmode_log, L_warning, "%N (%U@%H) enforces mode %c%c on %s.", cptr,
          cptr, cptr, (cmcptr->what == CHANMODE_ADD ? '+' : '-'),
...
/* -------------------------------------------------------------------------- *
 * -------------------------------------------------------------------------- */
struct node *chanmode_assemble(char *modebuf, char *parabuf, struct node *nptr,
                               size_t n, size_t count, int uid) {
  struct chanmodechange *cmcptr;
  char *arg;
  size_t mi;
  size_t pi;
...
void chanmode_send_local(struct client *cptr, struct channel *chptr,
  ...
  if (client_is_user(cptr)) {
    len = str_snprintf(cmd, sizeof(cmd), ":%s!%s@%s MODE %s", cptr->name,
                       cptr->user->name, cptr->host, chptr->name);
  } else {
    len =
        str_snprintf(cmd, sizeof(cmd), ":%s MODE %s", cptr->name, chptr->name);
  }
  ...
  for (node = nptr; node;) {
    ...
    if (flags[0]) {
      if (args[0]) {
        channel_send(NULL, chptr, CHFLG(NONE), CHFLG(NONE), "%s %s %s", cmd,
                     flags, args);
      } else {
        channel_send(NULL, chptr, CHFLG(NONE), CHFLG(NONE), "%s %s", cmd,
                     flags);
      }
    }
  }
...
void chanmode_send_remote(struct lclient *lcptr, struct client *cptr,
  ...
  if (client_is_user(cptr)) {
    len = str_snprintf(cmd, sizeof(cmd), ":%s MODE %s", cptr->user->uid,
                       chptr->name);
  } else {
    len =
        str_snprintf(cmd, sizeof(cmd), ":%s MODE %s", cptr->name, chptr->name);
  ...
  for (node = nptr; node;) {
    node = chanmode_assemble(flags, args, node, arglen, CHANMODE_PER_LINE,
                             CAP_UID);
    if (flags[0]) {
      if (args[0])
        server_send(lcptr, NULL, CAP_UID, CAP_NONE, "%s %s %s", cmd, flags,
                    args);
      else
        server_send(lcptr, NULL, CAP_UID, CAP_NONE, "%s %s", cmd, flags);
    }
  }
  ...
  for (node = nptr; node;) {
    ...
    if (flags[0]) {
      if (args[0]) {
        server_send(lcptr, NULL, CAP_NONE, CAP_UID, "%s %s %s", cmd, flags,
                    args);
      } else {
        server_send(lcptr, NULL, CAP_NONE, CAP_UID, "%s %s", cmd, flags);
      }
    }
  }
}
...
int chanmode_bounce_ban(struct lclient *lcptr, struct client *cptr,
  ...
  if (cmcptr->what == CHANMODE_ADD) {
    ...
    str_snprintf(cmcptr->arg, sizeof(cmcptr->arg), "%s!%s@%s", nick, user,
                 host);
  ...
  if (cmcptr->what == CHANMODE_ADD) {
    dlink_foreach_safe(&chptr->modelists[index], cmiptr, next) {
      ...
      if (str_match(cmiptr->mask, cmcptr->arg)) {
        chanmode_change_insert(lptr, cmcptr, CHANMODE_DEL, cmcptr->mode->letter,
                               cmiptr->mask);
...
int chanmode_mask_add(struct client *cptr, struct list *mlptr,
  ...
  if (cptr->user) {
    str_snprintf(cmiptr->info, sizeof(cmiptr->info), "%s!%s@%s", cptr->name,
...
/* -------------------------------------------------------------------------- *
 * -------------------------------------------------------------------------- */
void chanmode_changes_make(struct list *list, int what,
...
void chanmode_list(struct client *cptr, struct channel *chptr, char c) {
  ...
  dlink_foreach(&chptr->modelists[index], cmiptr) numeric_send(
      cptr, numeric, chptr->name, cmiptr->mask, cmiptr->info, cmiptr->ts);
...
struct node *chanmode_assemble_list(char *buf, struct node *nptr, size_t len) {
  ...
  do {
    ...
    if (lasthash && cmcptr->info && lasthash == cmcptr->ihash) {
      if (flen + ilen + tlen + mlen + 2 + 1 + 2 + 12 + str_len(cmcptr->arg) +
                  2 + 1 >
              len ||
      ...
      if (flen + ilen + tlen + mlen + 2 +
                  (cmcptr->info ? str_len(cmcptr->info) : 1) + 2 + 12 +
                  str_len(cmcptr->arg) + 2 + 1 >
              len ||
          flen == CHANMODE_PER_LINE)
        break;
      ...
      if (cmcptr->info[0]) {
        ilen +=
            strlcpy(&infobuf[ilen], cmcptr->info, sizeof(infobuf) - ilen - 1);
    ...
    if (cmcptr->ts) {
      if (lastts) {
        tlen += str_snprintf(&timebuf[tlen], sizeof(timebuf) - tlen - 1, "%li",
                             (long)(cmcptr->ts - lastts));
      } else {
        tlen += str_snprintf(&timebuf[tlen], sizeof(timebuf) - tlen - 1, "%lu",
                             (unsigned long)(cmcptr->ts));
        lastts = cmcptr->ts;
      }
    } else {
      timebuf[tlen++] = '-';
...
void chanmode_introduce(struct lclient *lcptr, struct client *cptr,
  ...
  len = str_snprintf(buf, sizeof(buf), ":%s NMODE %s %lu ", cptr->name,
...
size_t chanmode_burst(struct lclient *lcptr, struct channel *chptr) {
  ...
  debug(chanmode_log, "Bursting chanmodes for %s to %s", chptr->name,
        lcptr->name);
  ...
  for (i = 0; i < 0x40; i++) {
    ...
    dlink_foreach(&chptr->modelists[i], cmiptr) {
      cmcptr = chanmode_change_add(&modelist, CHANMODE_ADD, i + 0x40,
                                   cmiptr->mask, NULL);
  ...
  len = str_snprintf(buf, sizeof(buf), "NMODE %s %lu ", chptr->name,
...
void chanmode_drop(struct client *cptr, struct channel *chptr) {
  ...
  for (i = 0; i < 0x40; i++) {
    ...
    dlink_foreach(&chptr->modelists[i], cmiptr) {
      cmcptr = chanmode_change_add(&modelist, CHANMODE_DEL, i + 0x40,
                                   cmiptr->mask, NULL);
      mem_static_free(&chanmode_item_heap, cmiptr);
    }
