...
static void ircd_listen(const char *address, uint16_t port, const char *error) {
  log(ircd_log, L_fatal, "Could not bind to %s:%u: %s", address, (uint32_t)port,
      error);
  syscall_exit(1);
}
  ...
  if (days == 0) {
    if (hrs == 0) {
      if (mins == 0)
        ...
        str_snprintf(upstr, sizeof(upstr), "%u minutes, %u seconds", mins,
                     secs);
    } else {
...
void ircd_init(int argc, char **argv, char **envp) {
  log(ircd_log, L_startup, "*** Firing up %s v%s - %s ***", PACKAGE_NAME,
  ...
  log_source_filter =
      (~log_sources[ircd_log_in].flag) & (~log_sources[ircd_log_out].flag);
