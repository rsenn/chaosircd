...
/* ------------------------------------------------------------------------ */
int userdb_get_log() { return userdb_log; }
...
//  userdb->status = type;
  ...
  switch(userdb->query)
  {
    ...
    {
      log(userdb_log, L_verbose, "userdb verify (#%u) %s. %s",
    ...
    {
      log(userdb_log, L_verbose, "userdb search (#%u) %s. %s",
          userdb->id, what, userdb->uid);
    ...
    {
      log(userdb_log, L_verbose, "userdb mutate (#%u) %s. %s",
          userdb->id, what, userdb->uid);
...
//    if(userdb_child->status != CHILD_RUNNING)
    {
      ...
      {
        userdb_fds[CHILD_READ] = userdb_child->channels[0][CHILD_PARENT][CHILD_READ];
...
#else
        userdb_fds[CHILD_WRITE] = userdb_child->channels[0][CHILD_PARENT][CHILD_WRITE];
        io_queue_control(userdb_fds[CHILD_READ], ON, OFF, ON);
        io_queue_control(userdb_fds[CHILD_WRITE], OFF, ON, ON);
#endif /* HAVE_SOCKETPAIR */

        io_register(userdb_fds[CHILD_READ], IO_CB_READ, userdb_read, NULL);
      }
...
userdb_query_register(const char* uid, const char* values, void *callback, ...) {
  ...
  io_puts(userdb_fds[CHILD_WRITE], "userdb register %s %s", userdb->uid, userdb->params);
  ...
  log(userdb_log, L_verbose, "Started register query for %s (%s)", userdb->uid, userdb->params);
 ...
 * ------------------------------------------------------------------------ */
struct userdb *
userdb_vquery(int type, const char* uid, const char* params, void* callback, va_list args) {
  struct userdb *userdb;
  ...
  io_puts(userdb_fds[CHILD_WRITE], "userdb %s %s %s", userdb_query_names[userdb->query], userdb->uid, userdb->params);
            

  userdb->timer = timer_start(userdb_timeout, USERDB_TIMEOUT, userdb);

  timer_note(userdb->timer, "userdb timeout (%s %s %s)", userdb_query_names[userdb->query], userdb->uid, userdb->params);
...
userdb_query_search(const char* values, void *callback, ...) {
  va_list       args;
  va_start(args, callback);
  struct userdb* ret = userdb_vquery(USERDB_QUERY_SEARCH, values, "", callback, args);
  va_end(args);
  return ret;
}
...
userdb_query_mutate(const char* uid, const char* values, void *callback, ...) {
  va_list       args;
  va_start(args, callback);
  struct userdb* ret = userdb_vquery(USERDB_QUERY_MUTATE, uid, values, callback, args);
  va_end(args);
  return ret;
}
...
userdb_query_verify(const char* uid, const char* password, void *callback, ...) {
  va_list       args;
  va_start(args, callback);
  struct userdb* ret = userdb_vquery(USERDB_QUERY_VERIFY, uid, password, callback, args);
  va_end(args);
  return ret;
}
...
void userdb_dump(struct userdb *udptr)
{
  if(udptr == NULL)
  {
    ...
    dlink_foreach(&userdb_list, udptr)
    {
      if(udptr->query == USERDB_QUERY_VERIFY)
        dump(userdb_log, " #%u: [%u] %-20s (%p)",
             udptr->id, udptr->refcount, udptr->uid,
             udptr->callback);
    }
  ...
  else
  {
    ...
    dump(userdb_log, "       type: %s",
         udptr->query == USERDB_QUERY_VERIFY ? "verify" :
         udptr->query == USERDB_QUERY_REGISTER ? "register" :
         udptr->query == USERDB_QUERY_SEARCH ? "search" : "mutate");
    dump(userdb_log, "     status: %s", 
         udptr->status == USERDB_DONE ? "done" :
         udptr->status == USERDB_ERROR ? "error" : "timed out");
    dump(userdb_log, "      timer: %i", udptr->timer ? udptr->timer->id : -1);
    dump(userdb_log, "       args: %p, %p, %p, %p",
         udptr->args[0], udptr->args[1], udptr->args[2], udptr->args[3]);
