...
IRCD_DATA_DECL(struct capab server_caps[]) = {
    {"HUB", CAP_HUB}, {"EOB", CAP_EOB}, {"UID", CAP_UID}, {"KLN", CAP_KLN},
    {"GLN", CAP_GLN}, {"HOP", CAP_HOP}, {"PAR", CAP_PAR}, {"SPF", CAP_SPF},
    {"RW", CAP_RW},   {"DE", CAP_DE},   {"AM", CAP_AM},   {"TS", CAP_TS},
    {"NSV", CAP_NSV}, {"CLK", CAP_CLK}, {"SVC", CAP_SVC}, {"SSL", CAP_SSL},
    {NULL, 0},
};
...
/* -------------------------------------------------------------------------- *
 * -------------------------------------------------------------------------- */
static void server_format_cb(char **pptr, size_t *bptr, size_t n, int padding,
                             int left, void *arg) {
...
static void server_shift(int64_t delta) {
  ...
  dlink_foreach(&server_list, sptr) sptr->bstart += delta;
}
...
void server_shutdown(void) {
  ...
  dlink_foreach(&server_list, sptr) server_delete(sptr);
...
struct server *server_new_remote(struct lclient *lcptr, struct client *cptr,
  ...
  sptr->client =
      client_new_remote(CLIENT_SERVER, lcptr, NULL, cptr, sptr->name,
                        cptr->hops + 1, sptr->name, sptr->name, info);
...
struct server *server_find_id(uint32_t id) {
  ...
  dlink_foreach(&server_list, sptr) if (sptr->id == id) return sptr;
...
/* -------------------------------------------------------------------------- *
 * -------------------------------------------------------------------------- */
void server_vsend(struct lclient *one, struct channel *chptr, uint64_t caps,
                  uint64_t nocaps, const char *format, va_list args) {
  struct node *node;
...
void server_send(struct lclient *one, struct channel *chptr, uint64_t caps,
...
int server_relay(struct lclient *lcptr, struct client *cptr,
                 struct server *sptr, const char *format, char **argv) {
  if (sptr == server_me)
    return 0;

  client_send(sptr->client, format, cptr, argv[2], argv[3], argv[4], argv[5],
...
int server_relay_always(struct lclient *lcptr, struct client *cptr,
                        uint32_t sindex, const char *format, int *argc,
                        char **argv) {
  struct server *sptr;
  uint32_t i;
...
void server_send_pass(struct lclient *lcptr) {
  ...
  log(server_log, L_debug, "Sending password '%s' to %s.", ccptr->passwd,
      lcptr->connect->name);
}
...
void server_send_capabs(struct lclient *lcptr) {
  ...
  cap_can_send = server_default_caps | (conf_current.global.hub ? CAP_HUB : 0);
  enc_can_send = 0;
  ...
  log(server_log, L_debug, "Sending capabs '%s' to %s.", capbuf,
      lcptr->connect->name);
}
...
void server_send_server(struct lclient *lcptr) {
  lclient_send(lcptr, "SERVER %s :%s", client_me->name, client_me->info);

  log(server_log, L_debug, "Sending server '%s' (%s) to %s.", client_me->name,
      client_me->info, lcptr->connect->name);
}
...
void server_login(struct lclient *lcptr) {
  ...
  if (server_find_name(lcptr->name)) {
    log(server_log, L_warning, "Server %s already exists.", lcptr->name);
  ...
  if ((clptr = class_find_name(ccptr->class)) == NULL) {
    log(server_log, L_warning, "Invalid connection class: %s", ccptr->class);
  ...
  if (str_cmp(ccptr->passwd, lcptr->pass)) {
    log(server_log, L_warning, "Password mismatch for %s.", lcptr->name);
...
void server_progress(int fd, struct lclient *lcptr) {
  ...
  if (io_list[fd].sendq.size) {
    ...
    log(server_log, L_status,
        ...
        lcptr->server->out.servers, lcptr->server->out.clients,
        lcptr->server->out.channels, lcptr->server->out.chanmodes, lcptr->name);
    ...
    timer_note(lcptr->ptimer, "ping timer for server %s", lcptr->server->name);
  }
}
...
void server_register(struct lclient *lcptr, struct class *clptr) {
  ...
  server_send(lcptr, NULL, CAP_NSV, CAP_NONE, "NSERVER %S %s :%s", server_me,
              lcptr->name, lcptr->info);
  server_send(lcptr, NULL, CAP_NONE, CAP_NSV, "SERVER %s :%s", lcptr->name,
              lcptr->info);
...
void server_introduce(struct lclient *lcptr, struct server *sptr) {
  ...
  if (sptr != server_me && sptr->client != client_me && lcptr->server != sptr &&
      sptr->client->source != lcptr && lcptr != sptr->client->origin->source) {
    if (lcptr->caps & CAP_NSV) {
      lclient_send(lcptr, "NSERVER %N %S :%s", sptr->client->origin, sptr,
                   sptr->client->info);
    } else {
      if (server_is_local(sptr))
        lclient_send(lcptr, "SERVER %s :%s", sptr->name, sptr->client->info);
      else
        lclient_send(lcptr, ":%s SERVER %s :%s", sptr->client->origin->name,
...
void server_links(struct client *cptr, struct server *sptr) {
  ...
  numeric_send(cptr, RPL_LINKS, sptr->name, sptr->client->origin->name,
...
void server_burst(struct lclient *lcptr) {
  ...
  /* send start of burst */
  log(server_log, L_verbose, "Bursting %s to %s.", client_me->name,
      lcptr->name);
  ...
  dlink_foreach(&channel_list, chptr) lcptr->server->out.chanmodes +=
      channel_burst(lcptr, chptr);
  ...
  lclient_send(lcptr, "BURST %u %u %u %u %u", lcptr->server->out.sendq,
               lcptr->server->out.servers, lcptr->server->out.clients,
               lcptr->server->out.channels, lcptr->server->out.chanmodes);
...
void server_split(struct server *sptr, const char *uplink, const char *victim) {
  ...
  dlink_foreach_safe(&sptr->deps[CLIENT_USER], node, next) {
    ...
    chanuser_send(NULL, cptr, ":%s!%s@%s QUIT :%s %s", cptr->name,
                  cptr->user->name, cptr->host, uplink, victim);
  ...
  dlink_foreach_safe(&sptr->deps[CLIENT_SERVER], node, next) {
    ...
    /*    if(cptr == sptr->client)
        ...
        if(cptr->server == NULL)
        {
          log(server_log, L_warning, "split recursion error from %s",
       sptr->name); return;
        }*/
...
void server_exit(struct lclient *lcptr, struct client *cptr,
  ...
  if (client_is_local(sptr->client))
    log(lclient_log, L_status,
        "Local server %s exited: %s (send: %u.%03ukb recv: %u.%03ukb)",
        sptr->client->lclient->name, reason, sptr->client->lclient->sendk,
  ...
  if (reason) {
    if (cptr)
      server_send(lcptr, NULL, CAP_NONE, CAP_NONE, ":%N NQUIT %S :%s", cptr,
                  sptr, reason);
    else
      server_send(lcptr, NULL, CAP_NONE, CAP_NONE, "NQUIT %S :%s", sptr,
                  reason);
  } else {
    if (cptr)
      server_send(lcptr, NULL, CAP_NONE, CAP_NONE, ":%N NQUIT %S", cptr, sptr);
    else
      server_send(lcptr, NULL, CAP_NONE, CAP_NONE, "NQUIT %S", sptr);
  }
  ...
  if (cptr) {
    ...
    log(server_log, L_warning,
        "%s splitted. Lost %u servers, %u clients, %u channels.", sptr->name,
        split_servers, split_clients, split_channels);
  }
  ...
  if (sptr == NULL) {
    ...
    if (server_lists[SERVER_LOCAL].size) {
      ...
      dlink_foreach_data(&server_lists[SERVER_LOCAL], node, sptr)
          dump(server_log, " #%u: [%u] %-20s (%s)", sptr->id, sptr->refcount,
