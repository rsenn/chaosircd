...
/* -------------------------------------------------------------------------- *
 * -------------------------------------------------------------------------- */
struct cryptcap server_ciphers[] = { { "AES/128", CAP_ENC_AES_128, 16,
		CIPHER_AES_128 }, { "AES/192", CAP_ENC_AES_192, 24, CIPHER_AES_192 }, {
		"AES/256", CAP_ENC_AES_256, 32, CIPHER_AES_256 }, { "3DES/168",
		CAP_ENC_3DES_168, 24, CIPHER_3DES }, { "IDEA/128", CAP_ENC_IDEA_128, 16,
		CIPHER_IDEA }, { 0, 0, 0, 0 } };
...
struct server *server_new_remote(struct lclient *lcptr, struct client *cptr,
	...
	sptr->client = client_new_remote(CLIENT_SERVER, lcptr, NULL, cptr,
			sptr->name, cptr->hops + 1, sptr->name, sptr->name, info);
...
int server_relay_always(struct lclient *lcptr, struct client *cptr,
		uint32_t sindex, const char *format, int *argc, char **argv) {
	struct server *sptr;
	uint32_t i;
...
void server_progress(int fd, struct lclient *lcptr) {
	...
	if(io_list[fd].sendq.size) {
		...
		lcptr->ptimer = timer_start(server_ping, lcptr->class->ping_freq,
				lcptr);
	...
	if(sptr != server_me && sptr->client != client_me && lcptr->server != sptr
		...
		if(lcptr->caps & CAP_NSV) {
			...
			if(server_is_local(sptr))
				lclient_send(lcptr, "SERVER %s :%s", sptr->name,
						sptr->client->info);
			else
				lclient_send(lcptr, ":%s SERVER %s :%s",
						sptr->client->origin->name, sptr->name,
						sptr->client->info);
		}
	}
	...
	dlink_foreach_data(&sptr->deps[CLIENT_SERVER], nptr, acptr)
	{
		if(acptr->server == NULL) {
			log(server_log, L_warning,
					"invalid recursion while server_links on %s.", sptr->name);
			return;
		}
...
void server_exit(struct lclient *lcptr, struct client *cptr,
	...
	if(client_is_local(sptr->client))
		log(lclient_log, L_status,
				...
				(uint32_t) ((((double) sptr->client->lclient->sendb) * 1000)
						/ 1024), sptr->client->lclient->recvk,
				(uint32_t) ((((double) sptr->client->lclient->recvb) * 1000)
						/ 1024));

	if(reason) {
		if(cptr)
			server_send(lcptr, NULL, CAP_NONE, CAP_NONE, ":%N NQUIT %S :%s",
					cptr, sptr, reason);
		else
		...
		if(cptr)
			server_send(lcptr, NULL, CAP_NONE, CAP_NONE, ":%N NQUIT %S", cptr,
					sptr);
		else
	...
	if(cptr) {
		log(server_log, L_warning,
				...
				sptr->name, cptr->name, split_servers, split_clients,
				split_channels);
	} else {
		log(server_log, L_warning,
	...
	if(sptr == NULL) {
		...
		if(server_lists[SERVER_LOCAL].size) {
			...
			dlink_foreach_data(&server_lists[SERVER_LOCAL], node, sptr)
				dump(server_log, " #%u: [%u] %-20s (%s)", sptr->id,
						sptr->refcount,
		...
		dump(server_log, "    servers: %u links",
				sptr->deps[CLIENT_SERVER].size);
