...
void channel_delete(struct channel *chptr) {
	...
	dlink_delete(&channel_lists[chptr->hash % CHANNEL_HASH_SIZE],
			&chptr->hnode);
...
void channel_topic(struct lclient *lcptr, struct client *cptr,
	...
	if(hooks_call(channel_topic, HOOK_DEFAULT, lcptr, cptr, chptr, cuptr,
			topic))
		return;
	...
	if(str_ncmp(chptr->topic, topic, sizeof(chptr->topic) - 1)) {
		...
		/* Set topic info */
		if(client_is_user(cptr))
			str_snprintf(chptr->topic_info, sizeof(chptr->topic_info),
					"%s!%s@%s", cptr->name, cptr->user->name, cptr->host);
		else
...
void channel_message(struct lclient *lcptr, struct client *cptr,
	...
	if(client_is_user(cptr) || client_is_service(cptr))
		channel_send(lcptr, chptr, CHFLG(NONE), CHFLG(NONE),
				":%N!%U@%H %s %s :%s", cptr, cptr, cptr, cmd, chptr->name,
				text);
	else
		channel_send(lcptr, chptr, CHFLG(NONE), CHFLG(NONE), ":%N %s %s :%s",
	...
	dlink_foreach_data(&chptr->lchanusers, node, acuptr)
	{
		if(client_is_service(acuptr->client))
			service_handle(acuptr->client->service, lcptr, cptr, chptr, cmd,
					"%s", text);
	}

	server_send(lcptr, chptr, CAP_UID, CAP_NONE, ":%s %s %s :%s",
			cptr->user ? cptr->user->uid : cptr->name, cmd, chptr->name, text);
	server_send(lcptr, chptr, CAP_NONE, CAP_UID, ":%s %s %s :%s", cptr->name,
			cmd, chptr->name, text);
...
void channel_show(struct client *cptr) {
	...
	client_send(cptr, numeric_format(RPL_LISTSTART), client_me->name,
			cptr->name);
...
void channel_dump(struct channel *chptr) {
	if(chptr == NULL) {
		...
		dump(channel_log, "       lchanusers: %u nodes",
				chptr->lchanusers.size);
		dump(channel_log, "       rchanusers: %u nodes",
				chptr->rchanusers.size);
		dump(channel_log, "          invites: %u nodes",
				chptr->rchanusers.size);

		for(i = 0; i < sizeof(chptr->modelists) / sizeof(chptr->modelists[0]);
				i++)
