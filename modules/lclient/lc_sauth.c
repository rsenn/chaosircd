...
static void mo_proxy(struct lclient *lcptr, struct client *cptr, int argc,
                     char **argv);
...
static struct msg m_proxy_msg = {
    "PROXY", 1, 4, MFLG_OPER, {NULL, NULL, mo_proxy, mo_proxy}, mo_proxy_help};

/* -------------------------------------------------------------------------- *
 * Module hooks                                                               *
 * -------------------------------------------------------------------------- */
int lc_sauth_load(void) {
  if (hook_register(lclient_handshake, HOOK_DEFAULT, lc_sauth_handshake) ==
      NULL)
    return -1;
...
void lc_sauth_unload(void) {
  ...
  dlink_foreach_safe_data(&lc_sauth_list, node, next, arg) lc_sauth_done(arg);
...
static void lc_sauth_done(struct lc_sauth *arg) {
  ...
  if (arg->sauth_proxy.size) {
    dlink_foreach(&arg->sauth_proxy, nptr) sauth_delete(nptr->data);
...
static void lc_sauth_lookup_dns(struct lc_sauth *arg) {
  ...
  arg->sauth_dns =
      sauth_dns_reverse(arg->lclient->addr_remote, lc_sauth_dns, arg);

  /* Report start of the dns lookup */
  if (arg->sauth_dns) {
    lclient_send(arg->lclient,
                 ":%s NOTICE %s :(dns) looking up %u.%u.%u.%u.in-addr.arpa.",
                 lclient_me->name, arg->lclient->name, (uint32_t)ip[3],
                 (uint32_t)ip[2], (uint32_t)ip[1], (uint32_t)ip[0]);
  } else {
...
static int lc_sauth_lookup_auth(struct lc_sauth *arg) {
  /* Start AUTH query */
  arg->sauth_auth =
      sauth_auth(arg->lclient->addr_remote, arg->lclient->port_remote,
                 arg->lclient->port_local, lc_sauth_auth, arg);

  /* Report start of the auth lookup */
  if (arg->sauth_auth) {
    lclient_send(arg->lclient, ":%s NOTICE %s :(auth) checking %s:%u -> %s:%u.",
                 lclient_me->name, arg->lclient->name, arg->lclient->host,
                 arg->lclient->port_remote, lclient_me->name,
...
static int lc_sauth_check_proxy(struct lc_sauth *arg) {
  ...
  dlink_foreach(&m_proxy_list, nptr) {
    ...
    node->data = sauth_proxy(type, arg->lclient->addr_remote, port,
                             arg->lclient->addr_local, arg->lclient->port_local,
                             lc_sauth_proxy, arg);
  ...
  if (arg->sauth_proxy.size) {
    lclient_send(arg->lclient,
                 ":%s NOTICE %s :(proxy) initiating proxy check %s -> %s",
                 lclient_me->name, arg->lclient->name, arg->lclient->host,
...
void lc_sauth_auth(struct sauth *sauth, struct lc_sauth *arg) {
  if (sauth->ident[0]) {
    /* We got a reply, copy it to lclient struct and report success */
    strlcpy(arg->lclient->user->name, sauth->ident,
            sizeof(arg->lclient->user->name));
...
void lc_sauth_deny(struct lc_sauth *arg, uint16_t port, int type) {
  ...
  str_snprintf(msg, sizeof(msg), "open %s proxy on port %u", sauth_types[type],
               (uint32_t)port);
...
void lc_sauth_proxy(struct sauth *sauth, struct lc_sauth *arg) {
  ...
  lclient_send(arg->lclient, ":%s NOTICE %s :(proxy) %u/%s : %s",
               lclient_me->name, arg->lclient->name, sauth->remote,
  ...
  if (m_proxy_ini) {
    ...
    if ((m_proxy_ini = ini_find_name(M_PROXY_INI)) == NULL)
      log(lclient_log, L_status,
          "Could not find proxy database, add '" M_PROXY_INI
          "' to your config file.");
    else
      log(lclient_log, L_status, "Found proxy checklist: %s",
          m_proxy_ini->path);
  }
...
static void mo_proxy(struct lclient *lcptr, struct client *cptr, int argc,
  ...
  if (argc > 3) {
    if (argv[5]) {
      if (server_relay_maybe(lcptr, cptr, 2, ":%C PROXY %s %s %s :%s", &argc,
                             argv))
        return;
    } else if (argv[4]) {
      if (server_relay_maybe(lcptr, cptr, 2, ":%C PROXY %s %s :%s", &argc,
                             argv))
        return;
    } else {
  ...
  if (!str_icmp(argv[2], "list")) {
    ...
    client_send(cptr,
                ":%C NOTICE %C : ======= proxy checklist ======== ", client_me,
                cptr);
    ...
    dlink_foreach(&m_proxy_list, node) {
      client_send(cptr, ":%C NOTICE %C : %5u %s", client_me, cptr,
                  ((uint32_t)(size_t)node->data) >> 16,
                  sauth_types[((uint32_t)(size_t)node->data) & 0xffff]);
    }

    client_send(cptr,
                ":%C NOTICE %C : ==== end of proxy checklist ==== ", client_me,
                cptr);
  ...
  if (!str_icmp(argv[2], "add")) {
    ...
    client_send(cptr, ":%C NOTICE %C :*** added proxy check for %u/%s",
                client_me, cptr, (uint32_t)port, sauth_types[type]);
    ...
    client_send(cptr, ":%C NOTICE %C :*** invalid command: %s", client_me, cptr,
                argv[2]);
  }
}
