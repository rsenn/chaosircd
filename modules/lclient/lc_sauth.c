...
static void lc_sauth_dns(struct sauth *sauth, struct lc_sauth *arg);
static void lc_sauth_auth(struct sauth *sauth, struct lc_sauth *arg);
static void lc_sauth_proxy(struct sauth *sauth, struct lc_sauth *arg);
 ...
 * Message entries                                                            *
 * -------------------------------------------------------------------------- */
static char *mo_proxy_help[] = { "PROXY [server] <list|add|delete> [port] [type]", "",
        "Manages ports for the proxy scanner tests.", "", "list        Lists all scanned ports.",
        "add         Adds a port/service pair.", "delete      Deletes a port/service pair.", "",
        "Valid types are: http, socks4, socks5, wingate, cisco", "", "Examples:", "/PROXY add 8080 http",
        "/PROXY add 1080 socks4", "/PROXY add 23 wingate", "/PROXY add 23 cisco",
        NULL };

static struct msg m_proxy_msg = { "PROXY", 1, 4, MFLG_OPER, { NULL, NULL, mo_proxy, mo_proxy }, mo_proxy_help };

/* -------------------------------------------------------------------------- *
 * Module hooks                                                               *
 * -------------------------------------------------------------------------- */
int lc_sauth_load(void) {
	if(hook_register(lclient_handshake, HOOK_DEFAULT, lc_sauth_handshake) == NULL) return -1;
	...
	if((m_proxy_ini = ini_find_name(M_PROXY_INI)) == NULL) {
		log(lclient_log, L_warning, "Could not find proxy checklist, add '"M_PROXY_INI
		"' to your config file.");
	}

	if(m_proxy_ini) ini_callback(m_proxy_ini, m_proxy_callback);
...
static void lc_sauth_lookup_dns(struct lc_sauth *arg) {
	...
	arg->sauth_dns = sauth_dns_reverse(arg->lclient->addr_remote, lc_sauth_dns, arg);

	/* Report start of the dns lookup */
	if(arg->sauth_dns) {
		lclient_send(arg->lclient, ":%s NOTICE %s :(dns) looking up %u.%u.%u.%u.in-addr.arpa.", lclient_me->name,
		             arg->lclient->name, (uint32_t)ip[3], (uint32_t)ip[2], (uint32_t)ip[1], (uint32_t)ip[0]);
	} else {
		/* Huh, dns failed, maybe servauth done, let's try the auth query */
		lclient_send(arg->lclient, ":%s NOTICE %s :(dns) servauth down.", lclient_me->name, arg->lclient->name);
...
static int lc_sauth_lookup_auth(struct lc_sauth *arg) {
	/* Start AUTH query */
	arg->sauth_auth = sauth_auth(arg->lclient->addr_remote, arg->lclient->port_remote, arg->lclient->port_local,
	                             lc_sauth_auth, arg);

	/* Report start of the auth lookup */
	if(arg->sauth_auth) {
		lclient_send(arg->lclient, ":%s NOTICE %s :(auth) checking %s:%u -> %s:%u.", lclient_me->name,
		             arg->lclient->name, arg->lclient->host, arg->lclient->port_remote, lclient_me->name,
		             arg->lclient->port_local);
	} else {
		/* Huh, dns failed, maybe servauth done, let's register */
		lclient_send(arg->lclient, ":%s NOTICE %s :(auth) servauth down.", lclient_me->name, arg->lclient->name);
...
static int lc_sauth_check_proxy(struct lc_sauth *arg) {
	...
	dlink_foreach(&m_proxy_list, nptr)
	{
		...
		node->data = sauth_proxy(type, arg->lclient->addr_remote, port, arg->lclient->addr_local,
		                         arg->lclient->port_local, lc_sauth_proxy, arg);
	...
	if(arg->sauth_proxy.size) {
		lclient_send(arg->lclient, ":%s NOTICE %s :(proxy) initiating proxy check %s -> %s", lclient_me->name,
		             arg->lclient->name, arg->lclient->host, net_ntoa(arg->lclient->addr_local));
	} else {
		/* Huh, scan failed, maybe servauth done, let's register */
		lclient_send(arg->lclient, ":%s NOTICE %s :(proxy) servauth down.", lclient_me->name, arg->lclient->name);
...
void lc_sauth_dns(struct sauth *sauth, struct lc_sauth *arg) {
	if(sauth->host[0]) {
		...
		lclient_send(arg->lclient, ":%s NOTICE %s :(dns) resolved your address to %s.", lclient_me->name,
		             arg->lclient->name, arg->lclient->host);
	} else {
		/* Report failure */
		lclient_send(arg->lclient, ":%s NOTICE %s :(dns) could not resolve your address.", lclient_me->name,
		             arg->lclient->name, arg->lclient->host);
	}
...
void lc_sauth_auth(struct sauth *sauth, struct lc_sauth *arg) {
	if(sauth->ident[0]) {
		...
		lclient_send(arg->lclient, ":%s NOTICE %s :(auth) got ident response: %s", lclient_me->name, arg->lclient->name,
		             arg->lclient->user->name);
	} else {
		/* Report failure */
		lclient_send(arg->lclient, ":%s NOTICE %s :(auth) no ident response.", lclient_me->name, arg->lclient->name);
	}
...
void lc_sauth_deny(struct lc_sauth *arg, uint16_t port, int type) {
	...
	str_snprintf(msg, sizeof(msg), "open %s proxy on port %u", sauth_types[type], (uint32_t)port);
...
void lc_sauth_proxy(struct sauth *sauth, struct lc_sauth *arg) {
	...
	lclient_send(arg->lclient, ":%s NOTICE %s :(proxy) %u/%s : %s", lclient_me->name, arg->lclient->name, sauth->remote,
...
static struct node *m_proxy_find(uint16_t port, int service) {
	...
	dlink_foreach(&m_proxy_list, node)
	{
		...
		if((val >> 16) == port && (val & 0xffff) == (uint16_t)service) return node;
	}
	...
	if((isptr = ini_section_first(m_proxy_ini))) {
		do {
			...
			if(!ini_get_str(isptr, "type", typestr, 32)) {
				...
				if(type >= 0) {
					if(m_proxy_find(port, type) == NULL) m_proxy_add(port, type);
				}
			}
...
int m_proxy_cleanup(void) {
	if(m_proxy_ini) {
		if(m_proxy_save()) {
			log(lclient_log, L_warning, "Failed saving proxy checklist '%s'.", m_proxy_ini->name);
		}
	} else {
		if((m_proxy_ini = ini_find_name(M_PROXY_INI)) == NULL)
			log(lclient_log, L_status, "Could not find proxy database, add '"
...
void m_proxy_callback(struct ini *ini) {
	...
	if(m_proxy_load()) {
		log(client_log, L_warning, "Failed loading proxy checklist '%s'.", m_proxy_ini->name);
	}
}
...
static void mo_proxy(struct lclient *lcptr, struct client *cptr, int argc, char **argv) {
	...
	if(argc > 3) {
		if(argv[5]) {
			if(server_relay_maybe(lcptr, cptr, 2, ":%C PROXY %s %s %s :%s", &argc, argv)) return;
		} else if(argv[4]) {
			if(server_relay_maybe(lcptr, cptr, 2, ":%C PROXY %s %s :%s", &argc, argv)) return;
		} else {
			if(server_relay_maybe(lcptr, cptr, 1, ":%C PROXY %s :%s", &argc, argv)) return;
		}
	}
	...
	if(argv[3]) {
		...
		if(argv[4]) {
			...
			if(type == -1) {
				client_send(cptr, ":%C NOTICE %C :*** invalid proxy type: %s", client_me, cptr, argv[4]);
				return;
			}
		}
	}
	...
	if(!str_icmp(argv[2], "list")) {
		...
		client_send(cptr, ":%C NOTICE %C : ======= proxy checklist ======== ", client_me, cptr);
		client_send(cptr, ":%C NOTICE %C :  port service", client_me, cptr);
		client_send(cptr, ":%C NOTICE %C : ---------------------------- ", client_me, cptr);

		dlink_foreach(&m_proxy_list, node)
		{
			client_send(cptr, ":%C NOTICE %C : %5u %s", client_me, cptr, ((uint32_t)(size_t)node->data) >> 16,
			            sauth_types[((uint32_t)(size_t)node->data) & 0xffff]);
		}

		client_send(cptr, ":%C NOTICE %C : ==== end of proxy checklist ==== ", client_me, cptr);
	...
	if(!str_icmp(argv[2], "add")) {
		...
		if(m_proxy_find(port, type)) return;

		m_proxy_add(port, type);

		client_send(cptr, ":%C NOTICE %C :*** added proxy check for %u/%s", client_me, cptr, (uint32_t)port,
		            sauth_types[type]);
		...
		if(nptr == NULL) {
			client_send(cptr, ":%C NOTICE %C :*** no such proxy check: %u/%s", client_me, cptr, (uint32_t)port,
			            sauth_types[type]);
		} else {
			dlink_delete(&m_proxy_list, nptr);
			dlink_node_free(nptr);

			client_send(cptr, ":%C NOTICE %C :*** deleted proxy check for: %u/%s", client_me, cptr, (uint32_t)port,
			            sauth_types[type]);
		}

	} else {
		client_send(cptr, ":%C NOTICE %C :*** invalid command: %s", client_me, cptr, argv[2]);
	}
}
