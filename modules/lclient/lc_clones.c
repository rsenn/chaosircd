...
static int lc_clones_check_local(struct lclient *lcptr) {
  ...
  if (net > LC_CLONES_MAX_NET_LOCAL) {
    ...
    log(lclient_log, L_warning, "Local clone alert: %u clones from %s/24", net,
        net_ntoa(addr));
    ...
    log(lclient_log, L_warning, "Local clone alert: %u clones from %s", net,
        net_ntoa(addr));
...
static int lc_clones_check_remote(struct client *cptr) {
  ...
  if (net > LC_CLONES_MAX_NET_REMOTE) {
    ...
    log(lclient_log, L_warning, "Remote clone alert: %u clones from %s/24", net,
        net_ntoa(addr));
    ...
    log(lclient_log, L_warning, "Remote clone alert: %u clones from %s", net,
        net_ntoa(addr));
