 ...
 * Prototypes                                                                 *
 * -------------------------------------------------------------------------- */
static void ms_nclock(struct lclient *lcptr, struct client *cptr, int argc,
                      char **argv);
...
static char *ms_nclock_help[] = {
    "NCLOCK", "", "Synchronizes the clock between two servers.", NULL};

static struct msg ms_nclock_msg = {
    "NCLOCK", 0, 3, MFLG_SERVER, {NULL, NULL, ms_nclock, NULL}, ms_nclock_help};
...
static void ms_nclock(struct lclient *lcptr, struct client *cptr, int argc,
  ...
  if (argc == 3) {
    log(server_log, L_status, "Time synchronisation request from %s.",
        lcptr->name);

    lclient_send(lcptr, "NCLOCK %llu %lld", clk,
                 clk - timer_mtime - timer_offset);

    log(server_log, L_status, "Replying time delta %lld.",
        clk - timer_mtime - timer_offset);
  } else if (argc == 4) {
    lcptr->lag = timer_mtime + timer_offset - clk;

    lclient_send(lcptr, "NCLOCK %llu %lld :%llu", timer_mtime + timer_offset,
                 lcptr->lag >> 1,
                 timer_mtime + timer_offset + (lcptr->lag >> 1));
  } else if (argc == 5) {
    uint64_t tm;
    int64_t delta;
    ...
    log(server_log, L_status, "Timer delta: %lld New offset: %lld", delta,
        timer_offset);
