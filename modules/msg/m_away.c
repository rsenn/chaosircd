 ...
 * Prototypes                                                                 *
 * -------------------------------------------------------------------------- */
static void m_away(struct lclient *lcptr, struct client *cptr, int argc,
                   char **argv);
...
static char *m_away_help[] = {
    ...
    "To return to the normal state, use it without reason.", NULL};

static struct msg m_away_msg = {
    "AWAY", 0, 1, MFLG_CLIENT, {NULL, m_away, m_away, m_away}, m_away_help};
...
static void m_away(struct lclient *lcptr, struct client *cptr, int argc,
  ...
  if (argv[2] == NULL || argv[2][0] == '\0') {
    if (cptr->user->away[0]) {
      server_send(lcptr, NULL, CAP_UID, CAP_NONE, ":%s AWAY", cptr->user->uid);
      server_send(lcptr, NULL, CAP_NONE, CAP_UID, ":%s AWAY", cptr->name);
      cptr->user->away[0] = '\0';
  ...
  server_send(lcptr, NULL, CAP_UID, CAP_NONE, ":%s AWAY :%s", cptr->user->uid,
              cptr->user->away);
  server_send(lcptr, NULL, CAP_NONE, CAP_UID, ":%s AWAY :%s", cptr->name,
              cptr->user->away);
...
static void m_away_whois(struct client *cptr, struct user *auptr) {
  if (auptr->away[0])
    numeric_send(cptr, RPL_AWAY, auptr->client->name, auptr->away);
}
