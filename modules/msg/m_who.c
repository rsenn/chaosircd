 ...
 * Prototypes                                                                 *
 * -------------------------------------------------------------------------- */
static void m_who(struct lclient *lcptr, struct client *cptr, int argc,
                  char **argv);
static void ms_who(struct lclient *lcptr, struct client *cptr, int argc,
                   char **argv);
...
static char *m_who_help[] = {
    ...
    "forwarded to a remote server (RFC violating BitchX extension).", NULL};

static struct msg m_who_msg = {
    "WHO", 1, 3, MFLG_CLIENT, {NULL, m_who, ms_who, m_who}, m_who_help};
...
void m_who_unload(void) { msg_unregister(&m_who_msg); }
...
static void ms_who(struct lclient *lcptr, struct client *cptr, int argc,
  ...
  if ((asptr = server_find_name(argv[2])) == NULL) {
    log(server_log, L_warning, "Dropping WHO for unknown server %s.", argv[2]);
    return;
  }
  ...
  if (asptr == server_me) {
    ...
    if (argv[3])
      client_send(asptr->client, ":%C WHO %S :%s", cptr, asptr, argv[3]);
    else
      client_send(asptr->client, ":%C WHO %S", cptr, asptr);
  }
}
  ...
  if (argc > 3 && !str_icmp(argv[2], "-server")) {
    ...
    if ((cuptr = chanuser_find(chptr, cptr))) {
      dlink_foreach_data(&chptr->chanusers, node, acuptr) {
        ...
        numeric_send(cptr, RPL_WHOREPLY, chptr->name,
                     acuptr->client->user->name, acuptr->client->host,
                     acuptr->client->origin->name, acuptr->client->name,
                     acuptr->prefix[0] ? acuptr->prefix : "*",
                     acuptr->client->hops, acuptr->client->info);
    ...
    dlink_foreach(&acptr->user->channels, cuptr) {
      ...
      numeric_send(cptr, RPL_WHOREPLY, cuptr->channel->name, acptr->user->name,
                   acptr->host, acptr->origin->name, acptr->name,
                   cuptr->prefix[0] ? cuptr->prefix : "*", acptr->hops,
                   acptr->info);
    }
    ...
    dlink_foreach(&cptr->user->channels, cuptr) {
      ...
      numeric_send(cptr, RPL_WHOREPLY, "*", cuptr->client->user->name,
                   cuptr->client->host, cuptr->client->origin->name,
                   cuptr->client->name, "*", cuptr->client->hops,
                   cuptr->client->info);
    ...
    dlink_foreach_data(&client_lists[CLIENT_GLOBAL][CLIENT_USER], node, acptr) {
      ...
      numeric_send(cptr, RPL_WHOREPLY, "*", acptr->user->name, acptr->host,
                   acptr->origin->name, acptr->name, "*", acptr->hops,
                   acptr->info);
