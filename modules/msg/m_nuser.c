 ...
 * Prototypes                                                                 *
 * -------------------------------------------------------------------------- */
static void ms_nuser(struct lclient *lcptr, struct client *cptr, int argc,
                     char **argv);

/* -------------------------------------------------------------------------- *
 * Message entries                                                            *
 * -------------------------------------------------------------------------- */
static char *ms_nuser_help[] = {"NUSER <nick> <hops> <ts> <umodes> <user> "
                                "<host> <ip> <server> <uid> <info>",
                                "", "Introduces a client to a remote server.",
                                NULL};

static struct msg ms_nuser_msg = {
    "NUSER", 10, 10, MFLG_SERVER, {NULL, NULL, ms_nuser, NULL}, ms_nuser_help};
...
void m_nuser_unload(void) { msg_unregister(&ms_nuser_msg); }
...
static void ms_nuser(struct lclient *lcptr, struct client *cptr, int argc,
  ...
  if (!chars_valid_nick(argv[2])) {
    log(server_log, L_warning, "User has invalid nickname: %s", argv[2]);
  ...
  if (!chars_valid_user(argv[6])) {
    log(server_log, L_warning, "User %s has invalid username: %s", argv[2],
        argv[6]);
  ...
  if ((sptr = server_find_name(argv[9])) == NULL) {
    log(server_log, L_warning, "Client %s from invalid origin %s.", argv[2],
        argv[9]);
  ...
  if ((acptr = client_find_nick(argv[2]))) {
    ...
    if (acptr->ts <= ts) {
      ...
      client_send(cptr, ":%S NBOUNCE %s %s :%lu", server_me, argv[10], newnick,
                  ts);
  ...
  uptr->client =
      client_new_remote(CLIENT_USER, lcptr, uptr, sptr->client, argv[2],
                        atoi(argv[3]), argv[7], argv[8], argv[11]);
