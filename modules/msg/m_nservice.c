 ...
 * Prototypes                                                                 *
 * -------------------------------------------------------------------------- */
static void ms_nservice(struct lclient *lcptr, struct client *cptr, int argc,
                        char **argv);
...
static char *ms_nservice_help[] = {
    ...
    "Introduces a client to a remote server.", NULL};

static struct msg ms_nservice_msg = {
    "NSERVICE",      5, 5, MFLG_SERVER, {NULL, NULL, ms_nservice, NULL},
    ms_nservice_help};
...
void m_nservice_unload(void) { msg_unregister(&ms_nservice_msg); }
...
static void ms_nservice(struct lclient *lcptr, struct client *cptr, int argc,
  ...
  if (!chars_valid_nick(argv[2])) {
    log(server_log, L_warning, "service has invalid nickname: %s", argv[2]);
  ...
  if (!chars_valid_user(argv[3])) {
    log(server_log, L_warning, "service %s has invalid username: %s", argv[2],
        argv[5]);
