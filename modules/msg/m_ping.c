 ...
 * Prototypes                                                                 *
 * -------------------------------------------------------------------------- */
static void m_ping(struct lclient *lcptr, struct client *cptr, int argc,
                   char **argv);
...
static char *m_ping_help[] = {
    "PING [target] [text]", "", "Sends a irc ping to the target.",
    "If the target is ommited then it is sent to your local server.", NULL};

static struct msg m_ping_msg = {
    "PING", 0, 2, MFLG_CLIENT, {NULL, m_ping, m_ping, m_ping}, m_ping_help};
...
void m_ping_unload(void) { msg_unregister(&m_ping_msg); }
...
static void m_ping(struct lclient *lcptr, struct client *cptr, int argc,
  ...
  if (argc == 4 && chars_isdigit(argv[2][0])) {
    if ((asptr = server_find_name(argv[3])) == server_me) {
      client_send(cptr, ":%S PONG %S :%s", server_me, server_me, argv[2]);
      return;
    }
  }
