 ...
 * Prototypes                                                                 *
 * -------------------------------------------------------------------------- */
static void mo_module(struct lclient *lcptr, struct client *cptr, int argc,
                      char **argv);
...
static struct msg m_module_msg = {
    "MODULE",      1, 3, MFLG_OPER, {NULL, NULL, mo_module, mo_module},
    mo_module_help};
...
static void mo_module(struct lclient *lcptr, struct client *cptr, int argc,
  ...
  if (argc > 3) {
    if (argv[4]) {
      if (server_relay_maybe(lcptr, cptr, 2, ":%C MODULE %s %s :%s", &argc,
                             argv))
        return;
    } else {
  ...
  if (!str_icmp(argv[2], "list")) {
    client_send(
        cptr, ":%C NOTICE %C : ======= module list ======== ", client_me, cptr);

    client_send(cptr, ":%C NOTICE %C :  id name", client_me, cptr);
    client_send(cptr, ":%C NOTICE %C : ---------------------------- ",
                client_me, cptr);

    dlink_foreach_up(&module_list, module) {
      client_send(cptr, ":%C NOTICE %C : %3u %s", client_me, cptr, module->id,
                  module->name);
    }

    client_send(
        cptr, ":%C NOTICE %C : ==== end of module list ==== ", client_me, cptr);
  } else if (!str_icmp(argv[2], "load")) {
    if (argv[3] == NULL) {
      client_send(cptr, ":%C NOTICE %C : need path of the module", client_me,
                  cptr);
      return;
    }
    ...
    if (module) {
      client_send(cptr, ":%C NOTICE %C : loaded module '%s'.", client_me, cptr,
                  module->name);
    }
    ...
    if (module == NULL) {
      client_send(cptr, ":%C NOTICE %C : module '%s' not found.", client_me,
                  cptr, argv[3]);
      return;
    }

    if (module == m_module_module) {
      client_send(cptr, ":%C NOTICE %C : can not unload myself!", client_me,
                  cptr);
      return;
    }

    client_send(cptr, ":%C NOTICE %C : unloaded module '%s'.", client_me, cptr,
                module->name);
    ...
    if (module == m_module_module) {
      client_send(cptr, ":%C NOTICE %C : can not reload myself!", client_me,
                  cptr);
      return;
    }
    ...
    if (module_reload(module))
      ...
      client_send(cptr, ":%C NOTICE %C : reloaded module '%s'.", client_me,
                  cptr, module->name);
  } else {
