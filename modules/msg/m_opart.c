 ...
 * Prototypes                                                                 *
 * -------------------------------------------------------------------------- */
static void m_opart(struct lclient *lcptr, struct client *cptr, int argc,
                    char **argv);
static void ms_opart(struct lclient *lcptr, struct client *cptr, int argc,
                     char **argv);
...
static struct msg m_opart_msg = {"OPART",
                                 1,
                                 2,
                                 MFLG_CLIENT,
...
void m_opart_server_send(struct lclient *lcptr, struct chanuser *cuptr,
                         const char *reason) {
  if (reason) {
    server_send(lcptr, NULL, CAP_UID, CAP_NONE, ":%s OPART %s :%s",
                cuptr->client->user->uid, cuptr->channel->name, reason);
    server_send(lcptr, NULL, CAP_NONE, CAP_UID, ":%s OPART %s :%s",
                cuptr->client->name, cuptr->channel->name, reason);
  } else {
    server_send(lcptr, NULL, CAP_UID, CAP_NONE, ":%s OPART %s",
                cuptr->client->user->uid, cuptr->channel->name);
    server_send(lcptr, NULL, CAP_NONE, CAP_UID, ":%s OPART %s",
...
static void m_opart(struct lclient *lcptr, struct client *cptr, int argc,
  ...
  if (!channel_is_persistent(chptr)) {
    client_send(
        cptr,
        ":%S NOTICE %N :*** Channel %s needs to be persistent (+P) for /OPART",
  ...
  if (chptr->server == server_me) {
    log(m_opart_event_log, L_status, "%s destroyed event %s", cptr->name,
        chptr->name);
  }
...
static void m_opart_send(struct lclient *lcptr, struct client *optr,
  ...
  dlink_foreach_data(&chptr->lchanusers, node, cuptr) {
    ...
    if (acptr->lclient && lcptr == acptr->lclient) {
      if (reason && reason[0])
        client_send(acptr, ":%N!%U@%H PART %s :%s", acptr, acptr, acptr,
                    chptr->name, reason);
      else
        client_send(acptr, ":%N!%U@%H PART %s", acptr, acptr, acptr,
                    chptr->name);
    } else {
      if (reason && reason[0])
        client_send(acptr, ":%N!%U@%H KICK %s %N :%s", optr, optr, optr,
                    chptr->name, acptr, reason);
      else
        client_send(acptr, ":%N!%U@%H KICK %s %N", optr, optr, optr,
                    chptr->name, acptr);
    }
  }
}
