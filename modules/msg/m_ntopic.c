 ...
 * Prototypes                                                                 *
 * -------------------------------------------------------------------------- */
static void ms_ntopic(struct lclient *lcptr, struct client *cptr, int argc,
                      char **argv);
...
static char *ms_ntopic_help[] = {
    ...
    "Synchronizes channel topics with a remote server.", NULL};

static struct msg ms_ntopic_msg = {
    "NTOPIC", 4, 5, MFLG_SERVER, {NULL, NULL, ms_ntopic, NULL}, ms_ntopic_help};
...
void m_ntopic_unload(void) { msg_unregister(&ms_ntopic_msg); }
...
static void ms_ntopic(struct lclient *lcptr, struct client *cptr, int argc,
  ...
  if (chptr->topic[0])
    server_send(lcptr, NULL, CAP_NONE, CAP_NONE, ":%C NTOPIC %s %lu %s %lu :%s",
                cptr, chptr->name, chptr->ts, chptr->topic_info,
                chptr->topic_ts, chptr->topic);
  else
    server_send(lcptr, NULL, CAP_NONE, CAP_NONE, ":%C NTOPIC %s %lu %s %lu",
                cptr, chptr->name, chptr->ts, chptr->topic_info,
                chptr->topic_ts);

  if (changed) {
    channel_send(NULL, chptr, CHFLG(NONE), CHFLG(NONE), ":%C TOPIC %s :%s",
