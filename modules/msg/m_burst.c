 ...
 * Prototypes                                                                 *
 * -------------------------------------------------------------------------- */
static void ms_burst(struct lclient *lcptr, struct client *cptr, int argc,
                     char **argv);
...
static struct msg ms_burst_msg = {
    "BURST", 0, 0, MFLG_SERVER, {NULL, NULL, ms_burst, NULL}, ms_burst_help};
...
void m_burst_unload(void) { msg_unregister(&ms_burst_msg); }
...
static void ms_burst(struct lclient *lcptr, struct client *cptr, int argc,
  ...
  if (argc == 2) {
    if (client_is_local(cptr))
      ...
      log(server_log, L_status, "Start of burst from %N via %s", cptr,
          lcptr->name);
    ...
    server_send(lcptr, NULL, CAP_NONE, CAP_NONE, ":%N BURST", cptr);
  } else {
    ...
    if (cptr->server->in.servers != burst_servers)
      log(server_log, L_warning,
          "Server count doesn't match in burst from %N (r:%u/l:%u).", cptr,
          burst_servers, cptr->server->in.servers);
    if (cptr->server->in.clients != burst_clients)
      log(server_log, L_warning,
          "Client count doesn't match in burst from %N (r:%u/l:%u).", cptr,
          burst_clients, cptr->server->in.clients);
    if (cptr->server->in.channels != burst_channels)
      log(server_log, L_warning,
          "Channel count doesn't match in burst from %N (r:%u/l:%u).", cptr,
          burst_channels, cptr->server->in.channels);
    if (cptr->server->in.chanmodes != burst_chanmodes)
      log(server_log, L_warning,
          "Chanmode count doesn't match in burst from %N (r:%u/l:%u).", cptr,
          burst_chanmodes, cptr->server->in.chanmodes);
    ...
    if (client_is_local(cptr)) {
      ...
      log(server_log, L_status,
          "Burst from %N done in %llu msecs (%u.%02ukb/s)", cptr, burst_time,
          burst_kb, burst_db);

      log(server_log, L_status,
          "Synched %u servers, %u clients, %u channels, %u channelmodes from "
          "%N.",
          cptr->server->in.servers, cptr->server->in.clients,
          cptr->server->in.channels, cptr->server->in.chanmodes, cptr);
    } else {
      log(server_log, L_status, "Burst from %N via %s done in %llu msecs", cptr,
          lcptr->name, burst_time);
    ...
    server_send(lcptr, NULL, CAP_NONE, CAP_NONE, ":%N BURST %u %u %u %u %u",
                cptr, burst_sendq, cptr->server->in.servers,
                cptr->server->in.clients, cptr->server->in.channels,
