 ...
 * Prototypes                                                                 *
 * -------------------------------------------------------------------------- */
static void mo_gline(struct lclient *lcptr, struct client *cptr, int argc,
                     char **argv);
static void mo_ungline(struct lclient *lcptr, struct client *cptr, int argc,
                       char **argv);
static void ms_gline(struct lclient *lcptr, struct client *cptr, int argc,
                     char **argv);
static void ms_ungline(struct lclient *lcptr, struct client *cptr, int argc,
                       char **argv);
...
#endif
static void m_gline_mask(const char *host, net_addr_t *addr, net_addr_t *mask);
...
static struct m_gline_entry *m_gline_new(const char *user, const char *host,
  ...
  if (mgeptr->user[0] == '*' && mgeptr->user[1] == '\0' &&
    ...
    filter_rule_insert(m_gline_filter, FILTER_SRCNET, FILTER_DENY, mgeptr->addr,
                       mgeptr->mask, 0ull);
  ...
  if (reason) {
    ...
    debug(client_log, "new gline entry: %s@%s (%s)", mgeptr->user, mgeptr->host,
          mgeptr->reason);
  } else {
    mgeptr->reason[0] = '\0';
    debug(client_log, "new gline entry: %s@%s", mgeptr->user, mgeptr->host);
  }
...
static int m_gline_cleanup(void) {
  ...
  /* Otherwise try to create the INI */
    ...
    if ((m_gline_ini = ini_find_name(M_GLINE_INI)) == NULL)
      log(client_log, L_status,
          "Could not find gline database, add '" M_GLINE_INI
          "' to your config file.");
    else
 ...
 * -------------------------------------------------------------------------- */
static int m_gline_split(char user[IRCD_USERLEN], char host[IRCD_HOSTLEN],
                         char *mask) {
  char *p;
  size_t nonwild = 0;
  size_t wild = 0;
...
static int m_gline_hook(struct lclient *lcptr) {
  ...
  if ((mgeptr = m_gline_find(lcptr->user->name, lcptr->host,
  ...
  if (mgeptr) {
    ...
    if (mgeptr->addr != NET_ADDR_ANY)
      filter_rule_insert(m_gline_filter, FILTER_SRCNET, FILTER_DENY,
                         mgeptr->addr, mgeptr->mask, 0LLU);
    else
      filter_rule_insert(m_gline_filter, FILTER_SRCIP, FILTER_DENY,
...
static void m_gline_match(struct m_gline_entry *mgeptr) {
  ...
  dlink_foreach_safe_data(&lclient_lists[LCLIENT_USER], nptr, next, lcptr) {
    ...
    if (str_match(lcptr->user->name, mgeptr->user)) {
      if (((mgeptr->addr != net_addr_any) &&
           (mgeptr->addr & mgeptr->mask) ==
               (lcptr->addr_remote & mgeptr->mask)) ||
          str_imatch(lcptr->host, mgeptr->host) ||
          str_imatch(lcptr->hostip, mgeptr->host)) {
        ...
        if (mgeptr->reason[0]) {
          ...
          /* Reasonless g-line shouldn't be possible, but maybe we get such
           * remote */
...
static void mo_gline(struct lclient *lcptr, struct client *cptr, int argc,
  ...
  if (m_gline_find(user, host, addr)) {
    if (client_is_user(cptr))
      client_send(cptr,
                  ":%S NOTICE %C :*** There is already a g-line matching the "
                  "mask '%s@%s'",
  ...
  /* Create info string */
  if (client_is_user(cptr))
    snprintf(mask, sizeof(mask), "%s!%s@%s", cptr->name, cptr->user->name,
             cptr->host);
  else
  ...
  if (client_is_user(cptr))
    log(server_log, L_status, "%s (%s@%s) added a g-line for %s@%s (%s).",
        cptr->name, cptr->user->name, cptr->host, user, host, argv[3]);
  else
    log(server_log, L_status, "adding g-line for %s@%s (%s).", user, host,
        argv[3]);
  ...
  if (argv[3]) {
    server_send(NULL, NULL, CAP_GLN, CAP_NONE, ":%C GLINE %s %s %s %llu :%s",
                cptr, mgeptr->user, mgeptr->host, mgeptr->info, mgeptr->ts,
                mgeptr->reason);
  } else {
    server_send(NULL, NULL, CAP_GLN, CAP_NONE, ":%C GLINE %s %s %s %llu", cptr,
...
static void ms_gline(struct lclient *lcptr, struct client *cptr, int argc,
  ...
  if (client_is_server(cptr))
    log(server_log, L_verbose, "%s is bursting g-line for %s@%s.", cptr->name,
  ...
  /* Relay it further */
  server_send(lcptr, NULL, CAP_GLN, CAP_NONE, ":%C GLINE %s %s %s %llu", cptr,
              mgeptr->user, mgeptr->host, mgeptr->info, mgeptr->ts);
}
...
static void mo_ungline(struct lclient *lcptr, struct client *cptr, int argc,
  ...
  if ((mgeptr = m_gline_find(user, host, addr)) == NULL) {
    client_send(
        cptr, ":%S NOTICE %C :*** There is no g-line matching the mask '%s@%s'",
        server_me, cptr, user, host);
    return;
  }
  ...
  if (addr != NET_ADDR_ANY) {
    filter_rule_delete(m_gline_filter, FILTER_SRCNET, FILTER_DENY, addr,
                       netmask);
  ...
  log(server_log, L_status, "%s (%s@%s) removed g-line for %s@%s.", cptr->name,
      cptr->user->name, cptr->host, user, host);

  /* Relay it to all servers on the net */
  server_send(NULL, NULL, CAP_GLN, CAP_NONE, ":%C UNGLINE %s %s", cptr,
              mgeptr->user, mgeptr->host);
}
...
static void ms_ungline(struct lclient *lcptr, struct client *cptr, int argc,
  ...
  /* If the the source is a server then its a burst */
  if (client_is_server(cptr))
    log(server_log, L_status, "%s removed g-line for %s@%s.", cptr->name,
        mgeptr->user, mgeptr->host);
  else
  ...
  /* Relay it further */
  server_send(lcptr, NULL, CAP_GLN, CAP_NONE, ":%C UNGLINE %s %s", cptr,
              mgeptr->user, mgeptr->host);
}
...
static void m_gline_burst(struct lclient *lcptr) {
  ...
  dlink_foreach(&m_gline_list, mgeptr) {
    /* Burst a g-line */
    if (mgeptr->reason[0])
      lclient_send(lcptr, "GLINE %s %s %s %llu :%s", mgeptr->user, mgeptr->host,
                   mgeptr->info, mgeptr->ts, mgeptr->reason);
    else
      lclient_send(lcptr, "GLINE %s %s %s %llu", mgeptr->user, mgeptr->host,
                   mgeptr->info, mgeptr->ts);
  }
}
...
static void m_gline_stats(struct client *cptr) {
  ...
  dlink_foreach(&m_gline_list, mgeptr) {
    /* Send an entry */
    numeric_send(cptr, RPL_STATSGLINE, 'g', mgeptr->user, mgeptr->host,
                 (unsigned long)(mgeptr->ts / 1000L), mgeptr->info,
                 mgeptr->reason);
  }
}
