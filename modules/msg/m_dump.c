 ...
 * Prototypes                                                                 *
 * -------------------------------------------------------------------------- */
static void mo_dump(struct lclient *lcptr, struct client *cptr, int argc,
                    char **argv);
...
static char *mo_dump_help[] = {
    ...
    "Use without the <module> argument for a list of valid modules.", NULL};

static struct msg mo_dump_msg = {
    "DUMP", 0, 3, MFLG_OPER, {NULL, NULL, mo_dump, mo_dump}, mo_dump_help};
...
void m_dump_unload(void) { msg_unregister(&mo_dump_msg); }
...
static void m_dump_dlink(char *arg) { dlink_dump(); }
...
static struct {
  ...
  dump_cb_t *cb;
  int (*sp)(void);
} m_dump_table[] = {{"io", m_dump_io, io_get_log},
...
/* -------------------------------------------------------------------------- *
 * -------------------------------------------------------------------------- */
static void m_dump_callback(uint64_t flag, int lvl, const char *level,
                            const char *source, const char *date,
                            const char *msg, struct client *cptr) {
  client_send(cptr, ":%S NOTICE %C :%s", server_me, cptr, msg);
}
...
static void mo_dump(struct lclient *lcptr, struct client *cptr, int argc,
  ...
  if (argc == 2) {
    ...
    client_send(cptr, ":%S NOTICE %C :modules available to dump:", server_me,
                cptr);

    sz = (sizeof(m_dump_table) / sizeof(m_dump_table[0])) - 1;

    for (i = 0; i + 4 < sz; i += 4)
      client_send(cptr, ":%S NOTICE %C :%-10s %-10s %-10s %-10s", server_me,
                  cptr, m_dump_table[i + 0].name, m_dump_table[i + 1].name,
                  m_dump_table[i + 2].name, m_dump_table[i + 3].name);

    if (sz - i == 3)
      client_send(cptr, ":%S NOTICE %C :%-10s %-10s %-10s", server_me, cptr,
                  m_dump_table[i + 0].name, m_dump_table[i + 1].name,
                  m_dump_table[i + 2].name);
    else if (sz - i == 2)
      client_send(cptr, ":%S NOTICE %C :%-10s %-10s", server_me, cptr,
                  m_dump_table[i + 0].name, m_dump_table[i + 1].name);
    else if (sz - i == 1)
      client_send(cptr, ":%S NOTICE %C :%-10s", server_me, cptr,
