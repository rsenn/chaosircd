...
static void m_userdb(struct lclient *lcptr, struct client *cptr, int argc,
                     char **argv);
...
static struct msg m_userdb_msg = {
    "USERDB",     1, 0, MFLG_CLIENT, {NULL, m_userdb, m_userdb, m_userdb},
    m_userdb_help};

/* -------------------------------------------------------------------------- *
 * Module hooks                                                               *
 * -------------------------------------------------------------------------- */
int m_userdb_load(void) {
  /*  if(hook_register(lclient_handshake, HOOK_DEFAULT, m_userdb_handshake) ==
   * NULL)*/
  /*    return -1;*/
...
/* -------------------------------------------------------------------------- *
 * -------------------------------------------------------------------------- */
static struct m_userdb_s *m_userdb_new(struct lclient *lcptr) {
...
static void m_userdb_done(struct sauth *saptr, struct m_userdb_s *arg) {
  ...
  log(sauth_log, L_status, "userdb query (#%u) done: %s", arg->sauth->id,
      arg->sauth->userargs);

  lclient_send(arg->lclient, ":%S NOTICE %s :*** USERDB reply: %s %s",
               server_me, arg->lclient->name, arg->sauth->usercmd,
               arg->sauth->userargs);
...
/* -------------------------------------------------------------------------- *
 * -------------------------------------------------------------------------- */
static void m_userdb_callback(struct sauth *saptr) {}

/* -------------------------------------------------------------------------- *
 * Start an AUTH lookup for a local client                                    *
 * -------------------------------------------------------------------------- */
static struct m_userdb_s *m_userdb_query(struct lclient *lcptr, const char *cmd,
                                         const char *args) {
  ...
  if (arg->sauth) {
    lclient_send(arg->lclient, ":%s NOTICE %s :(userdb) query: %s",
