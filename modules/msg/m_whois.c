 ...
 * Prototypes                                                                 *
 * -------------------------------------------------------------------------- */
static void m_whois(struct lclient *lcptr, struct client *cptr, int argc,
                    char **argv);

/* -------------------------------------------------------------------------- *
 * Message entries                                                            *
 * -------------------------------------------------------------------------- */
static char *m_whois_help[] = {"WHOIS <nickname>",
                               "",
...
static struct msg m_whois_msg = {
    "WHOIS",     1, 2, MFLG_CLIENT, {NULL, m_whois, m_whois, m_whois},
    m_whois_help};
...
void m_whois_unload(void) { msg_unregister(&m_whois_msg); }
...
static void m_whois(struct lclient *lcptr, struct client *cptr, int argc,
  ...
  if (client_is_local(acptr)) {
    ...
    if (acptr->source->caps & CAP_UID)
      lclient_send(acptr->source, ":%s WHOIS %s", cptr->user->uid,
                   acptr->user->uid);
    else
      lclient_send(acptr->source, ":%s WHOIS %s", cptr->name, acptr->name);
  }
}
