 ...
 * Prototypes                                                                 *
 * -------------------------------------------------------------------------- */
static void m_ts(struct lclient *lcptr, struct client *cptr, int argc,
                 char **argv);
static void mo_ts(struct lclient *lcptr, struct client *cptr, int argc,
                  char **argv);
static void ms_ts(struct lclient *lcptr, struct client *cptr, int argc,
                  char **argv);

/* -------------------------------------------------------------------------- *
 * Message entries                                                            *
 * -------------------------------------------------------------------------- */
static char *mo_ts_help[] = {"TS [channel] [value]", "",
                             "Operator command setting the TS of a channel.",
                             NULL};

static struct msg mo_ts_msg = {
    "TS", 0, 2, MFLG_CLIENT, {NULL, m_ts, ms_ts, mo_ts}, mo_ts_help};
...
static void mo_ts(struct lclient *lcptr, struct client *cptr, int argc,
  ...
  log(channel_log, L_status,
      "%s (%s@%s) changed channel TS for %s from %u to %u.", cptr->name,
      cptr->user->name, cptr->host, chptr->name, chptr->ts, ts);

  chptr->ts = ts;

  server_send(NULL, NULL, CAP_TS, CAP_NONE, ":%C TS %s :%u", cptr, chptr->name,
              chptr->ts);
}
...
static void ms_ts(struct lclient *lcptr, struct client *cptr, int argc,
  ...
  server_send(lcptr, NULL, CAP_TS, CAP_NONE, ":%C TS %s :%u", cptr, chptr->name,
              chptr->ts);
}
