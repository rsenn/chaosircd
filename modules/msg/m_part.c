 ...
 * Prototypes                                                                 *
 * -------------------------------------------------------------------------- */
static void m_part(struct lclient *lcptr, struct client *cptr, int argc,
                   char **argv);
static void ms_part(struct lclient *lcptr, struct client *cptr, int argc,
                    char **argv);
...
static struct msg m_part_msg = {
    "PART",     1, 2, MFLG_CLIENT, {m_unregistered, m_part, ms_part, m_part},
    m_part_help};

/* -------------------------------------------------------------------------- *
 * -------------------------------------------------------------------------- */
#define channel_owner_flags                                                    \
  ((cuptr->flags & CHFLG(o)) | (cuptr->flags & CHFLG(h)) |                     \
   (cuptr->flags & CHFLG(v)))

#define chanuser_is_owner(cuptr)                                               \
  ((cuptr->flags & channel_owner_flags) == channel_owner_flags)

#define channel_is_persistent(chptr) ((chptr)->modes & CHFLG(P))
...
void m_part_unload(void) { msg_unregister(&m_part_msg); }
...
static void m_part(struct lclient *lcptr, struct client *cptr, int argc,
  ...
  if (chanuser_is_owner(cuptr) && client_is_local(cptr) &&
      channel_is_persistent(chptr))
  /*     !str_ncmp(cptr->name, &chptr->name[1], str_len(&chptr->name[1])))*/
  {
    client_send(
        cptr,
        ":%s NOTICE %N :*** You need to /OPART if you really want to leave %s.",
...
static void m_part_send(struct lclient *lcptr, struct client *cptr,
                        struct channel *chptr, const char *reason) {
  if (reason && reason[0])
    channel_send(NULL, chptr, CHFLG(NONE), CHFLG(NONE), ":%s!%s@%s PART %s :%s",
                 cptr->name, cptr->user->name, cptr->host, chptr->name, reason);
  else
    channel_send(NULL, chptr, CHFLG(NONE), CHFLG(NONE), ":%s!%s@%s PART %s",
                 cptr->name, cptr->user->name, cptr->host, chptr->name);
}
