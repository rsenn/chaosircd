...
static int cm_persistent_join_hook(struct lclient *lcptr, struct client *cptr,
	...
	if(owner) {
		if(created)
			log(cm_persistent_event_log, L_status, "%s created event %s",
					cptr->name, chptr->name);
		else
			log(cm_persistent_event_log, L_status, "%s rejoined event %s",
					cptr->name, chptr->name);
	} else if(channel_is_persistent(chptr))
		log(cm_persistent_participation_log, L_status,
				"%s accepted to help on %s", cptr->name, chptr->name);
	...
	if(chptr->server == server_me && (chptr->modes & CHFLG(P))) {
		...
		client_send(cptr, ":%S 611 %N %s :End of message replay", server_me,
				cptr, chptr->name);
	}
	return 0;
}
