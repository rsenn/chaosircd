...
static const char *cm_persistent_help[] = {
    "+P              Persistent channel. The channel will not be closed when "
    "the",
    "                last user parts. Channel messages will be kept and shown "
    "to",
    "                everyone who joins the channel.", NULL};
...
static int cm_persistent_join_hook(struct lclient *lcptr, struct client *cptr,
  ...
  owner =
      (0 == str_ncmp(cptr->name, &chptr->name[1], str_len(&chptr->name[1])));
  created = (chptr->chanusers.size <= 1);

  if (owner) {
    if (created)
      log(cm_persistent_event_log, L_status, "%s created event %s", cptr->name,
          chptr->name);
    else
      log(cm_persistent_event_log, L_status, "%s rejoined event %s", cptr->name,
          chptr->name);
  } else if (channel_is_persistent(chptr))
    log(cm_persistent_participation_log, L_status, "%s accepted to help on %s",
  ...
  if (chptr->server == server_me && (chptr->modes & CHFLG(P))) {
    ...
    dlink_foreach_down(&chptr->backlog, e) {
      if (e->text && e->text[0])
        client_send(cptr, ":%S 610 %N %s %u %s %s :%s", server_me, cptr,
                    chptr->name, e->ts, e->from, e->cmd, e->text);
      else
        client_send(cptr, ":%S 610 %N %s %u %s %s", server_me, cptr,
                    chptr->name, e->ts, e->from, e->cmd);
    }

    client_send(cptr, ":%S 611 %N %s :End of message replay", server_me, cptr,
                chptr->name);
  }
  return 0;
}
