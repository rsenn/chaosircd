...
int listen_collect(void) {
	...
	if(listen_dirty) {
		/* Report verbose */
		log(listen_log, L_verbose,
				"Doing garbage collect for [listen] module.");
...
void listen_dump(struct listen *lptr) {
	if(lptr == NULL) {
		...
		dlink_foreach(&listen_list, lptr)
			dump(listen_log, " #%03u: [%u] %-20s (%s:%u) (%s)", lptr->id,
					lptr->refcount, lptr->name, net_ntoa(lptr->addr_local),
