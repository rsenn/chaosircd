...
static size_t httpc_parse_protocol(struct httpc *hcptr) {
	...
	if((s = strstr(hcptr->url, "://"))) {
		for(i = 0;
				str_isalpha(hcptr->url[i]) && i < sizeof(hcptr->protocol) - 1;
				i++)
...
static size_t httpc_parse_port(struct httpc *hcptr, size_t index) {
	...
	if(hcptr->url[index] != ':') {
		...
		for(i = 0;
				str_isdigit(hcptr->url[index + i]) && i < sizeof(portnum) - 1;
				i++)
...
void httpc_var_build(struct httpc *hcptr) {
	...
	if(hcptr->content) {
		...
		dlink_foreach(&hcptr->vars, hvptr)
		{
			...
			n += httpc_url_encode(&hcptr->content[n], hvptr->name, size - n - 1,
					0);
			hcptr->content[n++] = '=';
			n += httpc_url_encode(&hcptr->content[n], hvptr->value,
					size - n - 1, 0);

			if(n + 1024 > size)
				hcptr->content = mem_dynamic_realloc(&httpc_dheap,
						hcptr->content, n + 1024);
		}
...
/* ------------------------------------------------------------------------ *
 * ------------------------------------------------------------------------ */
void httpc_header_set(struct httpc *hcptr, const char *name, const char *value) {
	struct httpc_var *hvptr;
	char var[64];
...
struct httpc *httpc_add(const char *url) {
	...
	HTTPC_TIMEOUT, HTTPC_INTERVAL, 0, hcptr->ssl, NULL)) == NULL) {
...
int httpc_url_build(struct httpc *hcptr) {
	...
	if(hcptr->type == HTTPC_TYPE_GET) {
		...
		if(hcptr->content_length) {
			hcptr->url[i++] = '?';
			i += strlcpy(&hcptr->url[i], hcptr->content,
					sizeof(hcptr->url) - i - 1);
		}
	}
