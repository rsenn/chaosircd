 ...
 * MA 02111-1307, USA
 *
 * $Id: graph.c,v 1.6 2006/09/28 09:38:20 roman Exp $
 */
...
//  width *= graph->samples;
	...
	graph->usamples = (graph->samples + 1) * (graph->resolution + 1)
			/ graph->unit;
	graph->usamples++;

	graph->data = mem_dynamic_alloc(&graph_data_heap,
			sizeof(double) * graph->samples);

	for(i = 0; i < graph->samples; i++)
		graph->data[i] = 0.0;

	graph->measure = mem_dynamic_alloc(&graph_data_heap,
			sizeof(char *) * graph->samples);
	graph->umeasure = mem_dynamic_alloc(&graph_data_heap,
			sizeof(char *) * graph->usamples);
	graph->mval = mem_dynamic_alloc(&graph_data_heap,
			sizeof(uint32_t) * graph->samples);
}
...
//  log(graph_log, L_verbose, "value: %u time: %u", (uint32_t)value, (uint32_t)time(NULL));
	...
	if(t > graph->current || t < graph->end) {
		log(graph_log, L_verbose,
				"Time mismatch: %u (Should be %u - %u) (Time: %u)", t,
				graph->end, graph->current, time(NULL));
...
#endif /* WIN32 */
		diff = (tm.tm_hour - g_tm.tm_hour) * 3600
				+ (tm.tm_min - g_tm.tm_min) * 60;
...
//    str_snprintf(m, sizeof(m), "%u", tm.tm_sec);
		...
		graph->umeasure[i] = mem_dynamic_alloc(&graph_data_heap,
				str_len(m) + 1);
...
void graph_draw_grid(struct graph *graph) {
	...
	for(i = 0; i < sizeof(graph_split) / sizeof(graph_split[0]); i++) {
		...
		if((val % graph_split[i])
				== 0&& (graph->grid_height / graph_split[i]) >= GRAPH_GRID_MINSPACE) {
			split = graph_split[i];
			break;
		}
	}
		...
		if(i < graph->samples - 1 && graph->mval[i + 1] > graph->mval[i]) {
			...
			if(i + 1 < graph->samples) {
				if(x1 + (w2 * 2) >= graph->width - GRAPH_PADDING_RIGHT)
					xpos2 = x1
							+ ((graph->width - GRAPH_PADDING_RIGHT) - x1) / 2;
	...
	/* zeichnet � chlin� querbauk� am r�cht� �nd vor x achs� */
	image_putvline(graph->image, x2, y2 - GRAPH_GRID_MEASURE,
			y2 + GRAPH_GRID_MEASURE,
			GRAPH_COLOR_BORDER);

	/* zeichnet � chlin� querbauk� am ober� �nd vor y achs� */
	image_puthline(graph->image, x1 - GRAPH_GRID_MEASURE,
			x1 + GRAPH_GRID_MEASURE, y1, GRAPH_COLOR_BORDER);
}
...
void graph_dump(struct graph *gptr) {
	if(gptr == NULL) {
		...
		dlink_foreach(&graph_list, gptr)
			dump(graph_log, " #%03u: [%u] (%u/%u) %-20s", gptr->id,
					gptr->refcount, gptr->resolution, gptr->samples,
					gptr->name);
