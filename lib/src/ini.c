 ...
 * MA 02111-1307, USA
 *
 * $Id: ini.c,v 1.5 2006/09/28 09:38:20 roman Exp $
 */
 ...
 * ------------------------------------------------------------------------ */
static inline struct ini_key *ini_key_get(struct ini_section *section,
		const char *name) {
	struct ini_key *key;
	struct node *node;
	hash_t hash;
 ...
 * ------------------------------------------------------------------------ */
static inline struct ini_key *ini_key_new(struct ini_section *section,
		const char *name) {
	struct ini_key *key;
...
void ini_init(void) {
	...
	mem_static_create(&ini_sec_heap, sizeof(struct ini_section),
			SEC_BLOCK_SIZE);
...
int ini_open(struct ini *ini, int mode) {
	ini->fd = io_open(ini->path, mode == INI_READ ? IO_OPEN_READ :
...
struct ini_section *ini_section_next(struct ini *ini) {
	...
	ini->current =
			ini->current->node.next ? ini->current->node.next->data : NULL;
...
void ini_dump(struct ini *ini) {
	...
	if(ini == NULL) {
		...
		dlink_foreach(&ini_list, ini)
		{
			dump(ini_log, " #%u: [%u] %-20s (%i)", ini->id, ini->refcount,
					ini->name, ini->fd);
		}
