...
void image_blit_32to32(struct image *src, struct rect *srect, struct image *dst,
	...
	srow =
			(struct color *) &src->pixel.data32[((sr.y * src->pitch) >> 2)
					+ sr.x];
	drow =
			(struct color *) &dst->pixel.data32[((dr.y * dst->pitch) >> 2)
					+ dr.x];

	for(y = 0; y < sr.h; y++) {
		for(x = 0; x < sr.w; x++) {
			drow[x].r = (srow[x].r * srow[x].a / 255)
					+ (drow[x].r * (255 - srow[x].a) / 255);
			drow[x].g = (srow[x].g * srow[x].a / 255)
					+ (drow[x].g * (255 - srow[x].a) / 255);
			drow[x].b = (srow[x].b * srow[x].a / 255)
					+ (drow[x].b * (255 - srow[x].a) / 255);
		}
...
static void image_alloc(struct image *iptr) {
	...
	iptr->pixel.data = mem_dynamic_alloc(&image_data_heap,
			iptr->pitch * iptr->h);
}
		...
		for(x = 0; x < iptr->w; x++) {
			...
			for(bitcount = 7; bitcount >= 0; bitcount--) {
				...
				if(current->children[key] == NULL) {
					...
					current->children[key] = mem_dynamic_alloc(&image_data_heap,
							sizeof(struct ctree));
					memset(current->children[key], 0, sizeof(struct ctree));
					current->children[key]->parent = current;

					node = dlink_node_new();
					dlink_add_tail(&levels[bitcount], node,
...
void image_putchar(struct image *iptr, struct font *ifptr, uint16_t x,
	...
	for(dy = 0; dy < ifptr->h; dy++) {
		for(dx = 0; dx < ifptr->w; dx++) {
			rowptr =
					(uint8_t *) &ifptr->data[((row + dy) * ifptr->w * 16) >> 3];
...
void image_dump(struct image *iptr) {
	if(iptr == NULL) {
		...
		dlink_foreach(&image_list, iptr)
			dump(image_log, " #%03u: [%u] %-20s", iptr->id, iptr->refcount,
					iptr->name);
