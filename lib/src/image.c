...
void image_init(void) {
  ...
  mem_dynamic_create(&image_palette_heap,
...
/* ------------------------------------------------------------------------ *
 * ------------------------------------------------------------------------ */
void image_blit_32to32(struct image *src, struct rect *srect, struct image *dst,
                       struct rect *drect) {
  struct color *srow;
  struct color *drow;
  uint16_t y;
  ...
  for (y = 0; y < sr.h; y++) {
    for (x = 0; x < sr.w; x++) {
      drow[x].r =
          (srow[x].r * srow[x].a / 255) + (drow[x].r * (255 - srow[x].a) / 255);
      drow[x].g =
          (srow[x].g * srow[x].a / 255) + (drow[x].g * (255 - srow[x].a) / 255);
      drow[x].b =
          (srow[x].b * srow[x].a / 255) + (drow[x].b * (255 - srow[x].a) / 255);
    }
...
struct image *image_new(int type, uint16_t width, uint16_t height) {
  ...
  image->pixel.data =
      mem_dynamic_alloc(&image_data_heap, image->opp * image->w * image->h);

  dlink_add_tail(&image_list, &image->node, image);

  memcpy(image->palette, image_defpal, sizeof(image->palette));

  log(image_log, L_status, "Added image block %ux%ux%u", image->w, image->h,
      image->bpp);
 ...
 * ------------------------------------------------------------------------ */
void image_release(struct image *iptr) { image_dirty = 1; }

/* ------------------------------------------------------------------------ *
 * ------------------------------------------------------------------------ */
void image_putpixel(struct image *iptr, int16_t x, int16_t y, uint32_t c) {
  if (x < 0 || x >= iptr->w || y < 0 || y >= iptr->h)
    return;
...
struct palette *image_quantize(struct image *iptr, int maxcolors, int *ckey) {
      ...
      for (bitcount = 7; bitcount >= 0; bitcount--) {
        ...
        if (current->children[key] == NULL) {
          ...
          if (++counts[bitcount] > (unsigned)maxcolors) {
            ...
            log(image_log, L_warning,
                "Color count exceeded, cropping to %u bits per channel.",
                8 - depth);
            break;
          }

          current->children[key] =
              mem_dynamic_alloc(&image_data_heap, sizeof(struct ctree));
  ...
  log(image_log, L_status,
      "Counted %u colors total while quantizing image %s (level %u).",
      counts[depth], iptr->name, depth);
  ...
  log(image_log, L_status, "Created a %u-color palette for image %s.",
      palette->count, iptr->name);
...
/* ------------------------------------------------------------------------ *
 * ------------------------------------------------------------------------ */
/*static*/ void image_32to8(struct image *iptr, struct palette *palette,
                            int colorkey) {
  struct color *c;
  uint32_t *olddata;
  uint32_t oldpitch;
...
/* ------------------------------------------------------------------------ *
 * ------------------------------------------------------------------------ */
void image_putindex(struct image *iptr, int16_t x, int16_t y, uint8_t i) {
  if (x < 0 || x >= iptr->w || y < 0 || y >= iptr->h)
    return;
  ...
  if (iptr->type == IMAGE_TYPE_8)
    ...
    iptr->pixel.data32[y * (iptr->pitch >> 2) + x] =
        *(uint32_t *)&iptr->palette[i];
}

/* ------------------------------------------------------------------------ *
 * ------------------------------------------------------------------------ */
void image_putcolor(struct image *iptr, int16_t x, int16_t y,
                    struct color *color) {
  if (x < 0 || x >= iptr->w || y < 0 || y >= iptr->h)
    return;
...
/* ------------------------------------------------------------------------ *
 * ------------------------------------------------------------------------ */
void image_puthline(struct image *iptr, int16_t x1, int16_t x2, int16_t y,
                    uint32_t c) {
  int16_t x;
...
/* ------------------------------------------------------------------------ *
 * ------------------------------------------------------------------------ */
void image_putvline(struct image *iptr, int16_t x, int16_t y1, int16_t y2,
                    uint32_t c) {
  int16_t y;
...
/* ------------------------------------------------------------------------ *
 * ------------------------------------------------------------------------ */
void image_putline(struct image *iptr, int16_t x1, int16_t y1, int16_t x2,
...
/* ------------------------------------------------------------------------ *
 * ------------------------------------------------------------------------ */
void image_putcircle(struct image *iptr, int16_t x, int16_t y, int rad,
                     int steps, uint32_t c) {
  int lastx;
  int lasty;
  double sx;
  double sy;
...
/* ------------------------------------------------------------------------ *
 * ------------------------------------------------------------------------ */
void image_putellipse(struct image *iptr, int16_t x, int16_t y, int xrad,
                      int yrad, int steps, uint32_t c) {
  int lastx;
  int lasty;
  double sx;
  double sy;
...
struct image *image_load_gif(const char *name) {
  ...
  if ((image = image_new(IMAGE_TYPE_8, gif->width, gif->height))) {
    ...
    if (gif->images.head) {
      ...
      for (i = 0; i < image->h; i++)
        memcpy(&image->pixel.data8[i * image->pitch],
               &img->bits[i * img->desc.width], img->desc.width);
...
struct image *image_pop(struct image *iptr) {
  if (iptr) {
    if (!iptr->refcount)
      log(image_log, L_warning, "Poping deprecated image: %s", iptr->name);
...
/* ------------------------------------------------------------------------ *
 * ------------------------------------------------------------------------ */
const char *image_get_name(struct image *image) { return image->name; }
...
void image_color_dump(struct color *color) {
  log(image_log, L_verbose, "r: %u g: %u b: %u a: %u", color->r, color->g,
      color->b, color->a);
}
...
void image_rect_dump(struct rect *rect) {
  log(image_log, L_verbose, "x: %i y: %i w: %u h: %u", rect->x, rect->y,
      rect->w, rect->h);
}
...
void image_palette_set(struct image *iptr, struct palette *pal) {
  ...
  log(image_log, L_status, "Set a %u-color palette for image %s.", pal->count,
      iptr->name);
}
 ...
 * ------------------------------------------------------------------------ */
struct palette *image_palette_make(uint32_t ncolors, struct color *colors) {
  struct palette *pal;
 ...
 * ------------------------------------------------------------------------ */
uint8_t image_palette_match(struct palette *palette, struct color *c,
                            int colorkey) {
  int i;
...
void image_dump(struct image *iptr) {
  if (iptr == NULL) {
    ...
    dlink_foreach(&image_list, iptr) dump(image_log, " #%03u: [%u] %-20s",
                                          iptr->id, iptr->refcount, iptr->name);
