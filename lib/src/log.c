...
const char *log_months[12] = { "Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul",
		"Aug", "Sep", "Oct", "Nov", "Dec", };
/* ------------------------------------------------------------------------ */
int log_get_log() {
	return log_log;
 ...
 * ------------------------------------------------------------------------ */
struct dlog *
log_drain_open(const char *path, uint64_t sources, int level, int prefix,
		int truncate) {
	struct dlog *drain;
	int fd;
	...
	dlink_foreach(&log_list, drain)
	{
		...
		if(drain->callback) {
			drain->callback(source->flag, level, log_levels[level][1],
					source->name, date, logmsg, drain->args[0], drain->args[1],
...
void log_drain_dump(struct dlog *dlptr) {
	if(dlptr == NULL) {
		...
		{
			if(dlptr->callback) {
				dump(log_log, " #%03u: [%u] (%s) %p ", dlptr->id,
						dlptr->refcount, log_levels[dlptr->level][1],
						dlptr->callback);
			} else {
				dump(log_log, " #%03u: [%u] (%s) %s ", dlptr->id,
						dlptr->refcount, log_levels[dlptr->level][1],
						dlptr->path);
			}
		}
		...
		dump(log_log, "       args: %p, %p, %p, %p", dlptr->args[0],
				dlptr->args[1], dlptr->args[2], dlptr->args[3]);
