...
const char *sauth_types[] = {"http",    "socks4", "socks5",
                             "wingate", "cisco",  NULL};

const char *sauth_replies[] = {"none",   "timeout", "filtered", "closed",
                               "denied", "n/a",     "open",     NULL};
...
static void sauth_callback(struct sauth *sauth, int type) {
  ...
  case SAUTH_TYPE_DNSF: {
    log(sauth_log, L_verbose, "DNS query (#%u) %s. (%s -> %s)", sauth->id, what,
        sauth->host,
  ...
  case SAUTH_TYPE_DNSR: {
    log(sauth_log, L_verbose, "DNS query (#%u) %s. (%s -> %s)", sauth->id, what,
        net_ntoa(sauth->addr), (sauth->host[0] ? sauth->host : "NXDOMAIN"));

    break;
  }
  case SAUTH_TYPE_AUTH: {
    log(sauth_log, L_verbose, "AUTH query (#%u) %s. (%s:%u -> %s)", sauth->id,
        what, net_ntoa(sauth->addr), sauth->local,
  ...
  case SAUTH_TYPE_PROXY: {
    ...
    log(sauth_log, L_verbose, "PROXY check (#%u) %s. (%s:%u -> %s:%u) [%s]: %s",
        sauth->id, what, net_ntoa(sauth->addr), (uint32_t)sauth->remote, buf,
        (uint32_t)sauth->local, sauth_types[sauth->ptype],
        sauth_replies[sauth->reply]);
    break;
  }
  case SAUTH_TYPE_USERDB: {
    /*log(sauth_log, L_verbose, "USERDB query (#%u) %s %-20s", sauth->id,
     * sauth->usercmd, sauth->userargs);*/
    break;
  }
  }
...
running:
  ...
  log(sauth_log, L_verbose, "Servauth child started (%u) (W=%u,R=%u)",
      sauth_child->pid, sauth_fds[CHILD_WRITE], sauth_fds[CHILD_READ]);
...
struct sauth *sauth_proxy(int type, net_addr_t remote_addr,
                          net_port_t remote_port, net_addr_t local_addr,
                          net_port_t local_port, void *callback, ...) {
  struct sauth *sauth;
  va_list args;
...
/* ------------------------------------------------------------------------ *
 * ------------------------------------------------------------------------ */
struct sauth *sauth_userdb(const char *cmd, const char *args, void *callback,
                           ...) {
  struct sauth *sauth;
  va_list arglist;
  ...
  io_puts(sauth_fds[CHILD_WRITE], "userdb %u %s %s", sauth->id, sauth->usercmd,
          sauth->userargs);

  log(sauth_log, L_verbose, "Started UserDB query: [%s] (#%u) %s",
      sauth->usercmd, sauth->id, sauth->userargs);
  ...
  if (saptr == NULL) {
    ...
    dlink_foreach(&sauth_list, saptr) {
      if (saptr->type == SAUTH_TYPE_DNSR)
        dump(sauth_log, " #%u: [%u] %-20s (%p)", saptr->id, saptr->refcount,
             net_ntoa(saptr->addr), saptr->callback);
    }
    ...
    dlink_foreach(&sauth_list, saptr) {
      if (saptr->type == SAUTH_TYPE_USERDB)
        dump(sauth_log, " #%u: [%u] %s %-20s (%p)", saptr->id, saptr->refcount,
             saptr->usercmd, saptr->userargs, saptr->callback);
    }
    ...
    dump(sauth_log, "       type: %s",
         ...
         : saptr->type == SAUTH_TYPE_AUTH ? "auth"
                                          : "proxy");
    dump(sauth_log, "     status: %s",
         saptr->status == SAUTH_DONE    ? "done"
         : saptr->status == SAUTH_ERROR ? "error"
                                        : "timed out");
