...
const char *sauth_types[] = { "http", "socks4", "socks5", "wingate", "cisco",
NULL };

const char *sauth_replies[] = { "none", "timeout", "filtered", "closed", "denied", "n/a", "open",
NULL };
...
int sauth_proxy_reply(const char *reply) {
	...
	for(i = 0; sauth_replies[i]; i++) {
		if(!str_icmp(sauth_replies[i], reply)) return i;
	}
...
int sauth_proxy_type(const char *type) {
	...
	for(i = 0; sauth_types[i]; i++) {
		if(!str_icmp(sauth_types[i], type)) return i;
	}
...
static void sauth_callback(struct sauth *sauth, int type) {
	...
	switch(sauth->type) {
		case SAUTH_TYPE_DNSF: {
			log(sauth_log, L_verbose, "DNS query (#%u) %s. (%s -> %s)", sauth->id, what, sauth->host,
			    (sauth->addr == NET_ADDR_ANY ? "NXDOMAIN" : net_ntoa(sauth->addr)));

			break;
		}
		case SAUTH_TYPE_DNSR: {
			log(sauth_log, L_verbose, "DNS query (#%u) %s. (%s -> %s)", sauth->id, what, net_ntoa(sauth->addr),
			    (sauth->host[0] ? sauth->host : "NXDOMAIN"));

			break;
		}
		case SAUTH_TYPE_AUTH: {
			log(sauth_log, L_verbose, "AUTH query (#%u) %s. (%s:%u -> %s)", sauth->id, what, net_ntoa(sauth->addr),
			    sauth->local, (sauth->ident[0] ? sauth->ident : "<unknown>"));
			break;
		}
		case SAUTH_TYPE_PROXY: {
			...
			log(sauth_log, L_verbose, "PROXY check (#%u) %s. (%s:%u -> %s:%u) [%s]: %s", sauth->id, what,
			    net_ntoa(sauth->addr), (uint32_t)sauth->remote, buf, (uint32_t)sauth->local, sauth_types[sauth->ptype],
	...
	if(sauth->callback) sauth->callback(sauth, sauth->args[0], sauth->args[1], sauth->args[2], sauth->args[3]);
}
...
int sauth_launch(void) {
	if(sauth_child == NULL) {
		...
		if(sauth_child == NULL) sauth_child = child_find_name("servauth");
		...
		if(sauth_child->status != CHILD_RUNNING) {
			...
			if(sauth_child->channels[0][CHILD_PARENT][CHILD_READ] == -1) return 0;
	...
	running: sauth_fds[CHILD_READ] = sauth_child->channels[0][CHILD_PARENT][CHILD_READ];
	...
	log(sauth_log, L_verbose, "Servauth child started (%u) (W=%u,R=%u)", sauth_child->pid, sauth_fds[CHILD_WRITE],
...
void sauth_collect(void) {
	...
	dlink_foreach(&sauth_list, node)
	{
		...
		if(!saptr->refcount) sauth_delete(saptr);
	}
...
sauth_dns_forward(const char *address, void *callback, ...) {
	...
	io_puts(sauth_fds[CHILD_WRITE], "dns forward %u %s", sauth->id, sauth->host);

	sauth->timer = timer_start(sauth_timeout, SAUTH_TIMEOUT, sauth);

	timer_note(sauth->timer, "sauth timeout (dns forward %s)", address);

	log(sauth_log, L_verbose, "Started DNS query for %s (#%u)", sauth->host, sauth->id);
...
sauth_dns_reverse(net_addr_t address, void *callback, ...) {
	...
	log(sauth_log, L_verbose, "Started DNS query for %s (#%u)", ipbuf, sauth->id);
 ...
 * ------------------------------------------------------------------------ */
struct sauth*
sauth_auth(net_addr_t address, uint16_t local, uint16_t remote, void *callback, ...) {
	struct sauth *sauth;
	va_list args;
	...
	io_puts(sauth_fds[CHILD_WRITE], "auth %u %s %u %u", sauth->id, net_ntoa(sauth->addr), sauth->local, sauth->remote);

	sauth->timer = timer_start(sauth_timeout, SAUTH_TIMEOUT, sauth);

	timer_note(sauth->timer, "sauth timeout (auth %s %u %u)", net_ntoa(sauth->addr), sauth->local, sauth->remote);
 ...
 * ------------------------------------------------------------------------ */
struct sauth*
sauth_proxy(int type, net_addr_t remote_addr, net_port_t remote_port, net_addr_t local_addr, net_port_t local_port,
            void *callback, ...) {
	struct sauth *sauth;
	va_list args;
	...
	io_puts(sauth_fds[CHILD_WRITE], "proxy %i %s:%u %s:%u %s", sauth->id, net_ntoa(sauth->addr), sauth->remote,
	        net_ntoa(sauth->connect), sauth->local, sauth_types[sauth->ptype]);

	sauth->timer = timer_start(sauth_timeout, SAUTH_TIMEOUT, sauth);

	timer_note(sauth->timer, "sauth timeout (proxy x %s:%u %s:%u %s)", net_ntoa(sauth->addr), sauth->remote,
	           net_ntoa(sauth->connect), sauth->local, sauth_types[sauth->ptype]);
...
sauth_userdb(const char* cmd, const char* args, void* callback, ...) {
	...
	io_puts(sauth_fds[CHILD_WRITE], "userdb %u %s %s", sauth->id, sauth->usercmd, sauth->userargs);
	...
	if(saptr == NULL) {
		...
		dlink_foreach(&sauth_list, saptr)
		{
			if(saptr->type == SAUTH_TYPE_DNSR)
			    dump(sauth_log, " #%u: [%u] %-20s (%p)", saptr->id, saptr->refcount, net_ntoa(saptr->addr),
			         saptr->callback);
		}
		...
		dlink_foreach(&sauth_list, saptr)
		{
			...
			dump(sauth_log, " #%u: [%u] %-20s (%p)", saptr->id, saptr->refcount, saptr->host, saptr->callback);
		}
		...
		dlink_foreach(&sauth_list, saptr)
		{
			if(saptr->type == SAUTH_TYPE_USERDB)
			    dump(sauth_log, " #%u: [%u] %s %-20s (%p)", saptr->id, saptr->refcount, saptr->usercmd, saptr->userargs,
			         saptr->callback);
		}
		...
		dump(sauth_log, "       args: %p, %p, %p, %p", saptr->args[0], saptr->args[1], saptr->args[2], saptr->args[3]);
