	...
	if(shptr == NULL) {
		...
		dlink_foreach(&mem_slist, shptr)
		{
			dump(mem_log, " #%u: [%2u x %2uk = %3uk] (%3u/%3u) %-20s",
					shptr->id, shptr->block_count, shptr->block_size / 1024,
			...
			total_usage += ((shptr->chunks_per_block * shptr->block_count)
					- shptr->free_chunks) * shptr->chunk_size;
		}
		...
		for(sbptr = shptr->base; sbptr; sbptr = sbptr->next) {
			dump(mem_log, "#%03u: %3u/%3u (%u/%u bytes)", i,
					sbptr->used_ones.size, shptr->chunks_per_block,
...
static void *mem_dynamic_block_alloc(struct dblock *dbptr, size_t size) {
	...
	if(dbptr->chunks.head == NULL) {
		...
		if(newchnk == NULL
				&& (size_t) end + sizeof(struct dchunk) + size
	...
	if(dhptr == NULL) {
		...
		dlink_foreach(&mem_dlist, dhptr)
		{
			dump(mem_log, " #%u: [%2u x %2uk = %3uk] (%3u/%3u:%3u) %-20s",
					dhptr->id, dhptr->block_count, dhptr->block_size / 1024,
					(dhptr->block_count * dhptr->block_size) / 1024,
					dhptr->used_bytes, dhptr->free_bytes + dhptr->used_bytes,
					dhptr->max_bytes, dhptr->note);
		}
		...
		for(dbptr = dhptr->base; dbptr; dbptr = dbptr->next) {
			dump(mem_log, "#%03u: %3u/%3u:%3u (%u chunks)", i,
					dbptr->used_bytes, dbptr->free_bytes + dbptr->used_bytes,
					dbptr->max_bytes, dbptr->chunks.size);
		}
...
void mem_static_create(struct sheap *shptr, size_t size, size_t count) {
	...
	blocksize = ((size + sizeof(struct schunk)) * count)
