		...
		dlink_foreach_safe(&fptr->rules, frptr, next)
		{
			if(frptr->ts && frptr->ts < timer_mtime) {
				...
				log(filter_log, L_verbose, "Deleted socket filter for %s",
						net_ntoa(*(net_addr_t *) &frptr->address));
...
static void filter_entry(struct filter *fptr, int type, uint32_t data1,
	...
	switch(type) {
		/*    case FILTER_PROTO:
		 ...
		 break;*/
		case FILTER_SRCIP:
			filter_ins(fptr, BPF_LD | BPF_W | BPF_ABS,
					SKF_NET_OFF + IPHDR_SRCIP);
			filter_jmp(fptr, BPF_JMP | BPF_JEQ | BPF_K, 0, 1, net_ntohl(data1));
			break;
		case FILTER_DSTIP:
			filter_ins(fptr, BPF_LD | BPF_W | BPF_ABS,
					SKF_NET_OFF + IPHDR_DSTIP);
			filter_jmp(fptr, BPF_JMP | BPF_JEQ | BPF_K, 0, 1, net_ntohl(data1));
			break;
		case FILTER_SRCNET:
			filter_ins(fptr, BPF_LD | BPF_W | BPF_ABS,
...
static void filter_ins(struct filter *fptr, uint16_t code, uint32_t k) {
	...
	prog->bf_insns = mem_dynamic_realloc(&filter_prog_heap, prog->bf_insns,
			sizeof(struct bpf_insn) * (prog->bf_len + 1));

	prog->bf_insns[prog->bf_len].code = code;
	prog->bf_insns[prog->bf_len].k = k;
	prog->bf_insns[prog->bf_len].jt = 0;
	prog->bf_insns[prog->bf_len].jf = 0;
...
static void filter_jmp(struct filter *fptr, uint16_t code, uint8_t jt,
	...
	prog->bf_insns[prog->bf_len].jf = jf;
	prog->bf_insns[prog->bf_len].k = k;
...
int filter_collect(void) {
	...
	if(filter_dirty) {
		/* Report verbose */
		log(filter_log, L_verbose,
				"Doing garbage collect for [filter] module.");
...
void filter_dump(struct filter *fptr) {
	if(fptr == NULL) {
		...
		dlink_foreach(&filter_list, fptr)
			dump(filter_log, " #%03u: [%u] %-20s", fptr->id, fptr->refcount,
					fptr->name);
		...
		dlink_foreach(&fptr->rules, rule)
		{
			...
			dump(filter_log, "   rule #%u: %s %s %s/%s", i,
					rule->type == FILTER_PROTO ? "PROTO" : (rule->type == FILTER_SRCIP ? "SRCIP" : (rule->type == FILTER_SRCNET ? "SRCNET" : (rule->type == FILTER_DSTIP ? "DSTIP" : "DSTNET"))),
		...
		if(prog->bf_len) {
			...
			for(i = 0; i < prog->bf_len; i++) {
				...
				dump(filter_log,
						(BPF_CLASS(p->code) == BPF_JMP && BPF_OP(p->code) != BPF_JA) ? "(%03d) %-8s %-16s jt %d\tjf %d" : "(%03d) %-8s %s",
