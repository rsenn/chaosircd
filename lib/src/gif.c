 ...
 * ------------------------------------------------------------------------ */
static uint8_t gif_compress_masks[] = { 0x00, 0x01, 0x03, 0x07, 0x0f, 0x1f,
		0x3f, 0x7f, 0xff };

static uint32_t gif_decompress_masks[] = { 0x0000, 0x0001, 0x0003, 0x0007,
		0x000f, 0x001f, 0x003f, 0x007f, 0x00ff, 0x01ff, 0x03ff, 0x07ff, 0x0fff };
	...
	while(i < len) {
		...
		if(crnt_code == eof_code) {
				...
				if(prefix[crnt_code] == GIF_NO_SUCH_CODE) {
					if(crnt_code == gif->lzw.running_code - 2) {
						...
						suffix[gif->lzw.running_code - 2] = stack[stack_ptr++] =
								gif_decompress_prefix(prefix, last_code,
										clear_code);
					} else {
						return -1;
					}
			...
			if(last_code != GIF_NO_SUCH_CODE) {
				...
				if(crnt_code == gif->lzw.running_code - 2)
					suffix[gif->lzw.running_code - 2] = gif_decompress_prefix(
							prefix, last_code, clear_code);
				else
					suffix[gif->lzw.running_code - 2] = gif_decompress_prefix(
							prefix, crnt_code, clear_code);
			}
...
void gif_init(void) {
	...
	mem_static_create(&gif_image_heap, sizeof(struct gif_image),
			GIF_BLOCK_SIZE);
	mem_static_note(&gif_image_heap, "gif image heap");
	mem_static_create(&gif_ht_heap, sizeof(struct gif_hashtable),
			GIF_BLOCK_SIZE);
...
int gif_screen_put(struct gif *gif, uint16_t width, uint16_t height,
	...
	if(gif_put_word(gif, width) || gif_put_word(gif, height)
			|| gif_put_byte(gif,
					(pal == NULL ? 0x00 : 0x80) | ((resolution - 1) << 4)
...
int gif_image_get(struct gif *gif) {
	...
	if(gif->desc.palette != NULL) {
		...
		image->desc.palette->colors = malloc(
...
int gif_image_put(struct gif *gif, int16_t left, int16_t top, uint16_t width,
	...
	if(gif_put_byte(gif, ',') || gif_put_word(gif, left)
			...
			|| gif_put_byte(gif,
					(pal == NULL ? 0x00 : 0x80) | (interlace ? 0x40 : 0x00)
							| (pal == NULL ? 0 : pal->bpp - 1)))
		return -1;
...
struct gif_image *gif_image_add(struct gif *gif, int16_t left, int16_t top,
		uint16_t width, uint16_t height, int interlace, struct palette *pal) {
	struct gif_image *image;
