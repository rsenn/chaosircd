 ...
 * MA 02111-1307, USA
 *
 * $Id: gif.c,v 1.5 2006/09/28 08:38:31 roman Exp $
 */
 ...
 * ------------------------------------------------------------------------ */
static uint8_t gif_compress_masks[] = {0x00, 0x01, 0x03, 0x07, 0x0f,
                                       0x1f, 0x3f, 0x7f, 0xff};
...
static int gif_write(struct gif *gif, const void *buf, size_t len) {
  ...
  /* Write to memory -> Realloc data space */
    ...
    /* Enlarge data space */
    if ((gif->mem = mem_dynamic_realloc(&gif_data_heap, gif->mem, newsize)) ==
        NULL)
      return -1;
...
static int gif_verify(struct gif *gif) {
  if (gif->status == GIF_STATE_READ) {
    ...
    if (gif_read(gif, buf, GIF_MAGIC_LEN) != GIF_MAGIC_LEN) {
      log(gif_log, L_warning, "No GIF signature on file '%s'.", gif->name);
      return -1;
    }
    ...
    if (str_cmp(buf, "GIF")) {
      log(gif_log, L_warning, "Invalid GIF signature on file '%s'.", gif->name);
      return -1;
    }
 ...
 * ------------------------------------------------------------------------ */
static int gif_ht_mask(unsigned long item) {
  return ((item >> 12) ^ item) & GIF_HT_KEY_MASK;
}

static void gif_ht_clear(struct gif_hashtable *table) {
...
static int gif_decompress_input(struct gif *gif, int *code) {
  ...
  *code = gif->lzw.shift_dword & gif_decompress_masks[gif->lzw.running_bits];
      ...
      if (crnt_code < clear_code) {
        ...
        if (prefix[crnt_code] == GIF_NO_SUCH_CODE) {
          if (crnt_code == gif->lzw.running_code - 2) {
            ...
            suffix[gif->lzw.running_code - 2] = stack[stack_ptr++] =
        ...
        while (j++ <= GIF_LZ_MAX_CODE && crnt_prefix > clear_code &&
...
static int gif_compress_output(struct gif *gif, int code) {
  ...
  if (code == GIF_FLUSH_OUTPUT) {
    while (gif->lzw.shift_state > 0) {
      if (gif_compress_buffer(gif, gif->lzw.buf, gif->lzw.shift_dword & 0xff))
        ret = -1;
...
void gif_init(void) {
  ...
  mem_dynamic_create(&gif_palette_heap,
                     (sizeof(struct palette) + sizeof(struct color) * 256) *
                         GIF_BLOCK_SIZE * 2);
...
struct gif *gif_open_file(const char *filename, uint32_t state) {
  ...
  fd = io_open(filename,
               (GIF_STATE_WRITE
                    ? (IO_OPEN_WRITE | IO_OPEN_TRUNCATE | IO_OPEN_CREATE)
                    : IO_OPEN_READ),
               0644);
...
int gif_screen_get(struct gif *gif) {
  ...
  if (gif_get_word(gif, &gif->width) || gif_get_word(gif, &gif->height)) {
 ...
 * ------------------------------------------------------------------------ */
int gif_screen_put(struct gif *gif, uint16_t width, uint16_t height,
                   int resolution, uint8_t background, struct palette *pal) {
  uint8_t buf[3];
  int i;
  ...
  if (gif_put_word(gif, width) || gif_put_word(gif, height) ||
      gif_put_byte(gif, (pal == NULL ? 0x00 : 0x80) | ((resolution - 1) << 4) |
                            (pal == NULL ? 0 : pal->bpp - 1)) ||
      gif_put_byte(gif, background) || gif_put_byte(gif, 0))
    return -1;
...
int gif_image_get(struct gif *gif) {
  ...
  if (gif_get_word(gif, (unsigned short *)&gif->desc.left) ||
      gif_get_word(gif, (unsigned short *)&gif->desc.top) ||
  ...
  if (gif->desc.palette != NULL) {
    ...
    image->desc.palette->colors =
        malloc(sizeof(struct color) * gif->desc.palette->count);
  ...
  gif->lzw.pixel_count = (uint32_t)gif->desc.width * (uint32_t)gif->desc.height;
 ...
 * Put an image                                                             *
 * ------------------------------------------------------------------------ */
int gif_image_put(struct gif *gif, int16_t left, int16_t top, uint16_t width,
                  uint16_t height, int interlace, struct palette *pal) {
  int i;
  uint8_t buf[3];
  ...
  if (gif_put_byte(gif, ',') || gif_put_word(gif, left) ||
      gif_put_word(gif, top) || gif_put_word(gif, width) ||
...
/* ------------------------------------------------------------------------ *
 * ------------------------------------------------------------------------ */
int gif_extension_put(struct gif *gif, uint8_t extcode, void *ext,
                      size_t extlen) {
  uint8_t buf[3];
...
/* ------------------------------------------------------------------------ *
 * ------------------------------------------------------------------------ */
int gif_get_gfx_control(struct gif_image *image, int *disposal, int *user_input,
                        int *trans, uint16_t *delay) {
  struct gif_ext *ext;
...
/* ------------------------------------------------------------------------ *
 * ------------------------------------------------------------------------ */
int gif_put_gfx_control(struct gif *gif, int disposal, int user_input,
                        int trans, uint16_t delay) {
  uint8_t ext[4];

  ext[0] = ((disposal & 0x07) << 1) | ((user_input & 0x01) << 1) |
...
int gif_get_record_type(struct gif *gif, enum gif_record *record) {
  ...
  switch (type) {
  case ',':
    *record = GIF_RECORD_IMAGE_DESC;
    break;
  case '!':
    *record = GIF_RECORD_EXTENSION;
    break;
  case ';':
    *record = GIF_RECORD_TERMINATE;
    break;
  default:
    *record = GIF_RECORD_UNDEFINED;
    return -1;
  }
...
int gif_slurp(struct gif *gif) {
  ...
  do {
    ...
    case GIF_RECORD_IMAGE_DESC: {
      ...
      image = (struct gif_image *)(gif->images.head ? gif->images.head->data
                                                    : NULL);
 ...
 * ------------------------------------------------------------------------ */
struct palette *gif_palette_make(uint32_t ncolors, struct color *colors) {
  struct palette *pal;
...
/* ------------------------------------------------------------------------ *
 * ------------------------------------------------------------------------ */
struct gif_image *gif_image_add(struct gif *gif, int16_t left, int16_t top,
...
void gif_extension_clear(struct gif_image *image) {
  ...
  dlink_foreach_safe(&image->exts, ext, next) gif_extension_delete(image, ext);
}

/* ------------------------------------------------------------------------ *
 * ------------------------------------------------------------------------ */
int gif_save(struct gif *gif) {
  gif->fd = io_open(gif->name,
