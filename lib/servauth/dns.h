...
struct dns_resolver {
  ...
  net_socket_t stype; /* socket type. SOCK_DGRAM or SOCK_STREAM */
  int retries;
  char type[2];
...
extern void dns_vset_option(int opt, va_list arg);
extern void dns_vget_option(int opt, va_list arg);
extern void *dns_get_userarg(struct dns_resolver *res);
extern void dns_set_userarg(struct dns_resolver *res, void *userarg);
...
extern void dns_set_callback(struct dns_resolver *res, dns_callback_t *callback,
                             uint64_t timeout);
extern void dns_forall_begin(void);
extern struct dns_resolver *dns_forall_next(void);
extern int dns_name_lookup(struct dns_resolver *res, int at, const char *name,
                           uint64_t t);
extern int dns_ptr_lookup(struct dns_resolver *res, int at, void *ip,
                          uint64_t t);
extern int dns_get_addr(struct dns_resolver *res, int at, void *addr);
extern int dns_get_addr_x(struct dns_resolver *res, int at, void *addr,
                          size_t nelem);
extern int dns_get_name(struct dns_resolver *res, char *buf, size_t n);
