 ...
 * start userdb client                                                *
 * -------------------------------------------------------------------------- */
int userdb_connect(struct userdb_client *userdb, const char *host,
...
int userdb_lookup(struct userdb_client *userdb, const char *uid) {
  userdb->result = db_query(
      userdb->handle, "SELECT * FROM users WHERE uid='%s' SORT BY uid;", uid);
...
/* -------------------------------------------------------------------------- *
 * -------------------------------------------------------------------------- */
static int build_values(struct db *db, stralloc *sa, char **v, size_t n,
                        char sep, char q, char stop, char start) {
  size_t i;
  ...
  for (i = 0; i < n; ++i) {
    ...
    if (sa->len)
      stralloc_catb(sa, &sep, 1);
    int quote =
        (q != '\0') &&
        (((sep == ',') || (!len || strchr(v[i], sep) != NULL)) || (q == '`'));
    if (quote)
      stralloc_catb(sa, &q, 1);
...
/* -------------------------------------------------------------------------- *
 * -------------------------------------------------------------------------- */
char **userdb_fields(struct userdb_client *userdb, size_t *nfields,
  ...
  struct db_result *result =
      db_query(userdb->handle, "SHOW FIELDS FROM users;");

  if (userdb->handle->error)
    return NULL;
  ...
  while ((row = db_fetch_row(result))) {
    ...
    if (exception && !str_cmp(f, exception))
      continue;
...
/* -------------------------------------------------------------------------- *
 * -------------------------------------------------------------------------- */
static char *get_varname(char *a, size_t *n) {
  char *p;
  if ((p = strchr(a, '='))) {
    *n = p - a;
    return a;
...
/* -------------------------------------------------------------------------- *
 * -------------------------------------------------------------------------- */
static char *get_varvalue(char *a) {
  char *p, *d;
  if ((p = strchr(a, '=')))
    ++p;
  else
...
/* -------------------------------------------------------------------------- *
 * -------------------------------------------------------------------------- */
static char *where_part(struct db *db, stralloc *sa, char **v,
                        size_t num_values) {
...
/* -------------------------------------------------------------------------- *
 * -------------------------------------------------------------------------- */
int userdb_search(struct userdb_client *userdb, char **v, size_t n,
                  char **retstr) {
  int ret = 0;
  size_t nfields;
  char **fields;
  ...
  *retstr = 0;
  ...
  struct db_result *result =
      db_query(userdb->handle, "SELECT %s FROM users WHERE %s;", sa.s, w);
  stralloc_free(&sa);
...
/* -------------------------------------------------------------------------- *
 * -------------------------------------------------------------------------- */
int userdb_verify(struct userdb_client *userdb, const char *uid,
                  const char *password, char **retstr) {
  int r;
  *retstr = 0;
  userdb->result =
      db_query(userdb->handle,
               "SELECT msisdn,imsi,imei,flag_level,last_login,last_location "
               "FROM chaosircd.users WHERE uid='%s' AND password='%s';",
               uid, password);
  ...
  if ((r = db_num_rows(userdb->result))) {
    ...
    build_values(userdb->handle, &sa, row, db_num_fields(userdb->result), ' ',
                 '\'', 0, 0);
    *retstr = sa.s;
    sa.s = 0;
  }
...
/* -------------------------------------------------------------------------- *
 * -------------------------------------------------------------------------- */
int userdb_register(struct userdb_client *userdb, char *uid, char **v,
                    size_t num_values) {
  ...
  db_query(userdb->handle, "INSERT INTO users (uid,%s) VALUES ('%s',%s);",
           fields.s, uid, values.s);
...
/* -------------------------------------------------------------------------- *
 * -------------------------------------------------------------------------- */
int userdb_mutate(struct userdb_client *userdb, const char *uid, char **v,
                  size_t num_values) {
  stralloc values;
