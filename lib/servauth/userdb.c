...
int userdb_lookup(struct userdb_client *userdb, const char* uid) {
	userdb->result = db_query(userdb->handle,
			"SELECT * FROM users WHERE uid='%s' SORT BY uid;", uid);
 ...
 * -------------------------------------------------------------------------- */
char**
userdb_fields(struct userdb_client *userdb, size_t* nfields,
		const char* exception) {

	if(nfields)
		*nfields = 0;

	struct db_result* result = db_query(userdb->handle,
...
int userdb_search(struct userdb_client *userdb, char** v, size_t n,
	...
	*retstr = 0;
	...
	struct db_result* result = db_query(userdb->handle,
			"SELECT %s FROM users WHERE %s;", sa.s, w);
	stralloc_free(&sa);
