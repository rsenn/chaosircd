...
static void query_dns_reverse_done(struct dns_resolver *res) {
	...
	/* Add to cache */
	cache_dns_put(&servauth_dnscache, CACHE_DNS_REVERSE, q->remote_addr,
			q->host, timer_systime);
...
static void query_auth_done(struct auth_client *auth) {
	...
	if(q->ident != NULL) {
		...
		/* add to cache */
		cache_auth_put(&servauth_authcache, status, q->remote_addr,
				timer_systime);
	}
...
int query_start(struct servauth_query *q, int type, uint64_t t) {
	...
	if(q != NULL) {
		...
		if(type & QUERY_AUTH) {
			...
			ret = auth_lookup(&q->auth, q->remote_addr, q->local_port,
					q->remote_port, t);
		}
