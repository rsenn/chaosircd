...
static const char *dns_errstrs[] = { "Success",
		"No domain name servers available", "Underlying syscall error",
		"Timed out", "Server failed", "Insufficient memory",
		"Fallback to IPv4 sockets", "Receive error", "Send error",
		"Could not connect" };
...
static int dns_extract_name(struct dns_resolver *res, char *out, size_t len) {
	...
	while(numans--) {
		...
		if(!memcmp(&header[0], DNS_T_PTR, 2))
			if(!memcmp(&header[2], DNS_C_IN, 2)) {
				if(!dns_packet_getname(res->reply, res->replylen, pos, name,
						255))
					return -1;
 ...
 *hsck = res->sock - 1;
 ...
 if(res->stype == NET_SOCKET_DGRAM && res->status != DNS_ST_SENT)
 {
...
int dns_name_lookup(struct dns_resolver *res, int at, const char *name,
	...
	type = (at == NET_ADDRESS_IPV4 ? DNS_T_A : DNS_T_AAAA);
