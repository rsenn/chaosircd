...
const char *proxy_types[] = {"http",    "socks4", "socks5",
                             "wingate", "cisco",  NULL};

const char *proxy_replies[] = {"none",   "timeout", "filtered", "closed",
...
static void proxy_event_rd(int fd, struct proxy_check *proxy) {
  ...
  case PROXY_TP_HTTP:
    ...
    if (n > 0 && (ptr = strstr(buf, "HTTP/1"))) {
      ...
      if ((num == 200 || num / 100 == 5) && num != 501 &&
          str_nicmp(&ptr[1], "ok", 2))
  ...
  case PROXY_TP_CISCO:
    ...
    if (!io_list[proxy_fd(proxy)].status.closed &&
        !io_list[proxy_fd(proxy)].status.err && (proxy->reply != PROXY_RP_OPEN))
      return;
...
static void proxy_event_wr(int fd, struct proxy_check *proxy) {
  ...
  case PROXY_TP_HTTP:
    io_puts(fd, "CONNECT %s:%u HTTP/1.0\r\n\r", net_ntoa(proxy->testaddr),
            (uint32_t)proxy->testport);
  ...
  case PROXY_TP_WINGATE:
    ...
    io_puts(fd, "%s:%u\r", net_ntoa(proxy->testaddr),
            (uint32_t)proxy->testport);
  ...
  case PROXY_TP_CISCO:
    ...
    io_puts(fd, "cisco\r");
    io_puts(fd, "telnet %s %u\r", net_ntoa(proxy->testaddr),
            (uint32_t)proxy->testport);
    proxy_send_irc(proxy);
    io_queue_control(proxy_fd(proxy), ON, OFF, ON);
    break;
...
int proxy_connect(struct proxy_check *proxy) {
  ...
  if (proxy->sock) {
    ...
    log(servauth_log, L_status, "connecting to %s:%u", net_ntoa(proxy->addr),
        (uint32_t)proxy->port);

    if (net_connect(proxy_fd(proxy), proxy->addr, proxy->port, proxy_event_cn,
                    proxy_event_cn, proxy))
      return -1;
...
/* -------------------------------------------------------------------------- *
 * -------------------------------------------------------------------------- */
int proxy_lookup(struct proxy_check *proxy, net_addr_t addr, uint16_t port,
                 net_addr_t testaddr, uint16_t testport, int type, uint64_t t) {
  proxy->addr = addr;
  proxy->port = port;
...
/* -------------------------------------------------------------------------- *
 * -------------------------------------------------------------------------- */
void *proxy_get_userarg(struct proxy_check *proxy) { return proxy->userarg; }
