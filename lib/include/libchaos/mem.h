 ...
 * ------------------------------------------------------------------------ */
#ifdef DEBUG
CHAOS_API(int mem_static_valid(struct sheap *shptr, void *scptr))
#endif
 ...
 * ------------------------------------------------------------------------ */
CHAOS_API(void *mem_dynamic_alloc(struct dheap *dhptr, size_t size))

/* ------------------------------------------------------------------------ *
 * Try to resize block, otherwise free and allocate new one.                  *
 * ------------------------------------------------------------------------ */
CHAOS_API(void *mem_dynamic_realloc(struct dheap *dhptr, void *ptr,
                                    size_t size))
 ...
 * ------------------------------------------------------------------------ */
#ifdef DEBUG
CHAOS_API(int mem_dynamic_valid(struct dheap *dhptr, void *dcptr))
#endif
...
CHAOS_API(inline  void *memset(void *s, int c, size_t n)
{
  ...
  /* n is a multiple of 4, so do 32bit copying */
    ...
    int32_t q = ((int32_t)(c & 0xff) << 0) | ((int32_t)(c & 0xff) << 8) |
                ((int32_t)(c & 0xff) << 16) | ((int32_t)(c & 0xff) << 24);
    n >>= 2;
  ...
  /* n is a multiple of 2, so do 16bit copying */
    ...
    int16_t q = ((int16_t)(c & 0xff) << 0) | ((int16_t)(c & 0xff) << 8);
    n >>= 1;
