...
/* ------------------------------------------------------------------------ *
 * ------------------------------------------------------------------------ */
typedef uint64_t log_source_t;

#define LOG_ALL (log_source_t)(int64_t)(-1)
#define LOG_SOURCE_COUNT (sizeof(uint64_t) << 3)
/* ------------------------------------------------------------------------ *
 * ------------------------------------------------------------------------ */
typedef void(log_drain_cb_t)(log_source_t src, int lvl, const char *level,
                             const char *source, const char *date,
                             const char *msg, void *arg0, void *arg1,
                             void *arg2, void *arg3);
 ...
 * ------------------------------------------------------------------------ */
CHAOS_API(void log_init(int fd, log_source_t sources, int level))

/* ------------------------------------------------------------------------ *
 * ------------------------------------------------------------------------ */
CHAOS_API(void log_level(log_source_t sources, int level))
...
/* ------------------------------------------------------------------------ *
 * ------------------------------------------------------------------------ */
CHAOS_API(struct dlog *log_drain_open(const char *path, log_source_t sources,
                                      int level, int prefix, int truncate))

/* ------------------------------------------------------------------------ *
 * ------------------------------------------------------------------------ */
CHAOS_API(struct dlog *log_drain_setfd(int fd, log_source_t sources, int level,
                                       int prefix))

/* ------------------------------------------------------------------------ *
 * ------------------------------------------------------------------------ */
CHAOS_API(struct dlog *log_drain_callback(void *callback, log_source_t sources,
                                          int level, ...))
...
/* ------------------------------------------------------------------------ *
 * ------------------------------------------------------------------------ */
CHAOS_API(struct dlog *log_drain_add(log_source_t sources, int level))

/* ------------------------------------------------------------------------ *
 * ------------------------------------------------------------------------ */
CHAOS_API(int log_drain_update(struct dlog *dlptr, log_source_t sources,
...
/* ------------------------------------------------------------------------ *
 * ------------------------------------------------------------------------ */
CHAOS_API(void log_drain_level(struct dlog *dlptr, int level))

/* ------------------------------------------------------------------------ *
 * Write a log line.                                                          *
 * ------------------------------------------------------------------------ */
#ifndef DEBUG
CHAOS_API(void log_output(log_source_t src, int level, const char *format, ...))
#endif
/* ------------------------------------------------------------------------ *
...
#ifdef _MSC_VER
#define fprintf(ios, ...) log(conf_log, L_warning, __VA_ARGS__)
#else
#define fprintf(ios, format...) log(conf_log, L_warning, format)
#endif
/*#if 1*/
...
#define log(src, level, format...)                                             \
  log_debug(__FILE__, __LINE__, (src), (level), format)
...
CHAOS_API(void log_debug(const char *file, int line, log_source_t src,
...
#define debug(src, ...) log_output((src), L_debug, __VA_ARGS__)

#ifdef _MSC_VER
#define dump(src, ...) log_output((src), L_debug, __VA_ARGS__)
#else
#define dump(src, format...) log_output((src), L_debug, format)
#endif
