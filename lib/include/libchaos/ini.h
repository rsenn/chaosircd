...
/* ------------------------------------------------------------------------ *
 * ------------------------------------------------------------------------ */
CHAOS_DATA(int) ini_log;
CHAOS_DATA(struct sheap) ini_heap;
CHAOS_DATA(struct sheap) ini_key_heap;
CHAOS_DATA(struct sheap) ini_sec_heap;
CHAOS_DATA(struct list) ini_list;
CHAOS_DATA(uint32_t) ini_serial;
CHAOS_DATA(struct timer *) ini_timer;
 ...
 * Open INI file.                                                           *
 * ------------------------------------------------------------------------ */
CHAOS_API(int ini_open(struct ini *ini, int mode))

/* ------------------------------------------------------------------------ *
 * Open INI file.                                                           *
 * ------------------------------------------------------------------------ */
CHAOS_API(int ini_open_fd(struct ini *ini, int fd))
 ...
 * Setup callback.                                                          *
 * ------------------------------------------------------------------------ */
CHAOS_API(void ini_callback(struct ini *ini, ini_callback_t *cb))
 ...
 * Delete a section.                                                        *
 * ------------------------------------------------------------------------ */
CHAOS_API(void ini_section_remove(struct ini *ini, struct ini_section *section))
 ...
 * ------------------------------------------------------------------------ */
CHAOS_API(int ini_write_str(struct ini_section *section, const char *key,
                            const char *str))

CHAOS_API(int ini_write_int(struct ini_section *section, const char *key,
                            int i))

CHAOS_API(int ini_write_ulong_long(struct ini_section *section, const char *key,
                                   uint64_t u))

CHAOS_API(int ini_write_double(struct ini_section *section, const char *key,
                               double d))

/* ------------------------------------------------------------------------ *
 * Read data from .ini, returning -1 when key not found.                      *
 * ------------------------------------------------------------------------ */
CHAOS_API(int ini_read_str(struct ini_section *section, const char *key,
                           char **str))

CHAOS_API(int ini_get_str(struct ini_section *section, const char *key,
                          char *str, size_t len))

CHAOS_API(int ini_read_int(struct ini_section *section, const char *key,
                           int *i))

CHAOS_API(int ini_read_ulong_long(struct ini_section *section, const char *key,
                                  uint64_t *u))

CHAOS_API(int ini_read_double(struct ini_section *section, const char *key,
                              double *d))

/* ------------------------------------------------------------------------ *
 * Read data from .ini, defaulting to a given value.                          *
 * ------------------------------------------------------------------------ */
CHAOS_API(int ini_read_str_def(struct ini_section *section, const char *key,
                               char **str, char *def))

CHAOS_API(int ini_get_str_def(struct ini_section *section, const char *key,
                              char *str, size_t len, char *def))

CHAOS_API(int ini_read_int_def(struct ini_section *section, const char *key,
                               int *i, int def))

CHAOS_API(int ini_read_double_def(struct ini_section *section, const char *key,
                                  double *d, double def))

CHAOS_API(int ini_read_color(struct ini_section *section, const char *key,
                             struct color *color))
