...
int64 io_mmapwritefile(int64 out, int64 in, uint64 off, uint64 bytes,
...
#ifdef __MINGW32__
    if (!e->mh)
      e->mh = CreateFileMapping(out, 0, PAGE_READONLY, 0, 0, NULL);
    if (!e->mh)
      goto readwrite;
#endif
    do {
...
#ifdef __MINGW32__
      if ((e->mmapped =
               MapViewOfFile(e->mh, FILE_MAP_READ, (DWORD)(e->mapofs >> 32),
                             (DWORD)e->mapofs, e->maplen)) == 0)
#else
      if ((e->mmapped = mmap(0, e->maplen, PROT_READ, MAP_SHARED, in,
                             e->mapofs)) == MAP_FAILED)
#endif
      {
        e->mmapped = 0;
