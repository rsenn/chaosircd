  ...
  if (x[i].type == FROMBUF) {
    ...
    if (x + i + 1 < last && (x[i + 1].type == FROMFILE)) {
      ...
      if (!TransmitFile(s, (HANDLE)x[i].fd,
                        x[i].n + tfb.HeadLength > 0xffff ? 0xffff : x[i].n, 0,
                        &e[i].os, &tfb, TF_USE_KERNEL_APC)) {
    ...
    e[i].os.Offset = x[i].offset;
    e[i].os.OffsetHigh = (x[i].offset >> 32);
    if (!TransmitFile(s, (HANDLE)x[i].fd, x[i].n > 0xffff ? 0xffff : x[i].n, 0,
                      &e[i].os, 0, TF_USE_KERNEL_APC))
      return -3;
    return sent;
  }
}
...
int64 iob_send(int64 s, io_batch *b) {
  ...
  for (;;) {
    ...
    if (e[i].type == FROMFILE) {
      ...
      r = sendfile(e[headers].fd, s, e[headers].offset, e[headers].n, &hdr,
                   &sbytes, 0);
      if (r == 0)
        sent = b->bytesleft;
