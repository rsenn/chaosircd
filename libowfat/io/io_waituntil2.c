...
int64 io_waituntil2(int64 milliseconds) {
  ...
  if (io_waitmode == _SIGIO) {
    ...
    io_entry *e;
    if (alt_firstread >= 0 &&
        (e = array_get(&io_fds, sizeof(io_entry), alt_firstread)) && e->canread)
      return 1;
    if (alt_firstwrite >= 0 &&
        (e = array_get(&io_fds, sizeof(io_entry), alt_firstwrite)) &&
        e->canwrite)
      return 1;
    if (milliseconds == -1)
      ...
      if (r == io_signum) {
        ...
        if (e) {
          ...
          if (info.si_band & POLLIN && !e->canread) {
            debug_printf(("io_waituntil2: enqueueing %ld in normal read queue "
                          "before %ld\n",
          ...
          if (info.si_band & POLLOUT && !e->canwrite) {
            debug_printf(("io_waituntil2: enqueueing %ld in normal write queue "
                          "before %ld\n",
...
dopoll:
  ...
  if (first_readable != -1 || first_writeable != -1) {
    fprintf(stderr,
            "io_waituntil2() returning immediately because first_readable(%p) "
            "or first_writeable(%p) are set\n",
            first_readable, first_writeable);
    return;
  }
  fprintf(stderr, "Calling GetQueuedCompletionStatus %p...", io_comport);
  if (GetQueuedCompletionStatus(io_comport, &numberofbytes, &x, &o,
                                milliseconds == -1 ? INFINITE : milliseconds)) {
    ...
    fprintf(stderr, "o=%p, e->or=%p, e->ow=%p, e->os=%p\n", o, &e->or, &e->ow,
            &e->os);
    fprintf(stderr,
            "e->readqueued=%d, e->writequeued=%d, e->acceptqueued=%d, "
            "e->connectqueued=%d, e->sendfilequeued=%d\n",
            e->readqueued, e->writequeued, e->acceptqueued, e->connectqueued,
            e->sendfilequeued);
