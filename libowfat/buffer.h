...
#define BUFFER_INIT(op, fd, buf, len)                                          \
  { (buf), 0, 0, (len), (fd), (op), NOTHING, NULL }
#define BUFFER_INIT_FREE(op, fd, buf, len)                                     \
  { (buf), 0, 0, (len), (fd), (op), FREE }
...
#define buffer_PUTC(s, c)                                                      \
  (((s)->a != (s)->p) ? ((s)->x[(s)->p++] = (c), 0) : buffer_put((s), &(c), 1))
...
int buffer_get_new_token_sa(buffer *b, stralloc *sa, const char *charset,
                            size_t setlen);
