...
__static inline __gnuinline int range_ptrinbuf(const void *buf, size_t len,
                                               const void *ptr) {
  ...
  return __likely(
      c && /* is buf non-NULL? */
      ((uintptr_t)c) + len >
          (uintptr_t)c && /* gcc 4.1 miscompiles without (uintptr_t) */
      ...
      Now, ptr-c can underflow; say it is -1.  When we cast it to uintptr_t, it
...
__static inline __gnuinline int range_validbuf2(const void *Min,
                                                const void *Max) {
  return __likely(Min && Max >= Min);
}

/* is buf2[0..len2-1] inside buf1[0..len-1]? */
__static inline __gnuinline int range_bufinbuf(const void *buf1, size_t len1,
