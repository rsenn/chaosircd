...
int main(int argc, char *argv[]) {
  ...
  unsigned long offset, endoffset, totalsize, linelen, part,
      reconstructed = 0; /* used only for yenc */
  ...
  for (;;) {
    ...
    hiteof:
      ...
      if (!found)
        buffer_putsflush(
            buffer_2,
            "warning: hit end of file without finding any uuencoded data!\n");
      return 0;
    }
    ++lineno;
    if (l > 0 && line[l - 1] == '\r')
    ...
    if (str_start(line, "begin ")) {
      ...
      case MIME:
        switch (mimeenc) {
        case BASE64:
          x = scan_base64(line, tmp, &scanned);
          break;
        case QP:
          x = scan_quotedprintable(line, tmp, &scanned);
          break;
        default:
          break;
        }
