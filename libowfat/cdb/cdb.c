...
void cdb_findstart(struct cdb *c) { c->loop = 0; }
...
void cdb_init(struct cdb *c, int64 fd) {
  ...
  {
    HANDLE m =
        CreateFileMapping((HANDLE)(uintptr_t)fd, 0, PAGE_READONLY, 0, 0, NULL);
    if (m)
...
static int match(struct cdb *c, const unsigned char *key, unsigned long int len,
                 uint32 pos) {
...
int cdb_findnext(struct cdb *c, const unsigned char *key,
