...
int cdb_make_addend(struct cdb_make *c, unsigned long int keylen,
                    unsigned long int datalen, uint32 h) {
...
int cdb_make_addbegin(struct cdb_make *c, unsigned long int keylen,
...
int cdb_make_add(struct cdb_make *c, const unsigned char *key,
                 unsigned long int keylen, const unsigned char *data,
                 unsigned long int datalen) {
  if (cdb_make_addbegin(c, keylen, datalen) == -1)
    return -1;
  if (buffer_putalign(&c->b, (char *)key, keylen) == -1)
    return -1;
  if (buffer_putalign(&c->b, (char *)data, datalen) == -1)
    return -1;
